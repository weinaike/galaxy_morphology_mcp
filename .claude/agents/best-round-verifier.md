...
|---|---|---|
| **5a single-band Disk profile class** | With ≥ 2 central components, the Disk component's type must be `expdisk` (not `sersic`) named `disk`; a `singlesersic` (sersic with free n) is legal only as the **sole** luminous component — a lingering `singlesersic` alongside others is equally invalid. | Violation (sersic disk / lingering singlesersic) → **FAIL** |
...
| **5c Concentricity of central components** | All main-galaxy central components' (Disk, Bulge, Bar, Lens, OuterDisk, **and the AGN psf point core**) final fitted `xcen` and `ycen` must be exactly identical (single-band: coordinates from the galfit.NN converged rows). With ≥ 2 main-galaxy central members **counting the AGN** (e.g. `{disk, agn}`, `{singlesersic, agn}`, `{disk, agn, bulge}`), the `.cons` pointed to by the feedme `G)` must contain the **paired** chain `offset` lines (`<chain> x offset` / `<chain> y offset`) **including the AGN's component number** — separate ±2px centre windows instead of the chain are a process violation. Companion numbers (`companion`/`comp`/`secondary`/`satellite`, **psf or sersic**) must NEVER appear in the chain — a psf companion is not the main-galaxy point core and keeps its free ±5px window. In the archived galfit.NN the chained subordinates' `1)` toggles should read `2 2` — GALFIT's constrained marker; still `1 1` means the constraint was silently ignored. | Non-concentric → **FAIL**; single-band missing the paired offset chain (incl. the AGN number when it is a chain member) or toggles not `2 2` → **WARN** |
