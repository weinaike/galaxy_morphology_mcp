...
| AGN | `psf` | ≤ 1 | Bulge Re collapsed < 0.2 px (mandatory replacement); 0.2–0.5 px border zone as a competing variant; with a non-collapsed bulge present, an obvious positive-residual leftover within 0–5 px of the 1D profile that the Bulge cannot absorb (a healthy bulge does not close the door); or, on a SingleSersic-only model (elliptical regime, no bulge exists), a compact central positive residual / 1D spike within <5 px surviving an otherwise clean single-Sérsic fit. The AGN is the **sole** allowed central point-source component — any other point-source component name is outside the solution space and rejected |
...
| Single Sersic | `sersic` (n free), named `singlesersic` | exactly 1; legal co-tenants: the AGN point core (`{singlesersic, agn}` = the elliptical+point-core terminal state) and companions (`{singlesersic, companion}` — field contaminants, not main-galaxy structure) | elliptical terminal state; if it fits q < 0.5, the classification must be revisited (a disk is implied). **Transitions**: single→multi — the first `add(<central component>)` (disk/bulge/bar/lens/outerdisk) MUST bundle the conversion `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68)` (the `disk` name is reserved for the expdisk slot; a multi-component model with a sersic `disk` or a `singlesersic` alongside anything but the AGN and companions is invalid — `check_feedme_file` rejects it); `add(agn)` and `add(companion)` are exempt from the conversion (neither is a central component; `{singlesersic, agn}` is legal without any conversion — the expert-typical elliptical+point-core answer, KILOGAS_221 retrospective; likewise `{singlesersic, companion}` for an elliptical with a field companion); multi→single — remove-only transitions leaving one central component may revert `disk → singlesersic` (n free) |

**Regime rule (hard; the formal combination space)**: the legal inventories are exactly the union of two regimes —
- **single-Sérsic regime**: `singlesersic + (AGN)? + (companion)*` — `{singlesersic}`, `{singlesersic, agn}`, plus any number of companions;
- **multi-component regime**: `(outerdisk)? + (disk | edgedisk) + (lens)? + (bar)? + (bulge)? + (AGN)? + (companion)*` — the disk/edgedisk slot is **mandatory** (the decomposition regime is disk-bearing by definition), F1 ≤ 1 rides on the disk/edgedisk (or the singlesersic in the other regime), the sky is exactly 1 in both.
A diskless multi-component inventory (e.g. `{bulge, companion}`, `{bar, agn}`) is **invalid** — it belongs to the single-Sérsic regime; both the candidate schema (`E_MULTIPLICITY`) and `check_feedme_file` reject it.
...
| n (Sérsic) | Disk: none (expdisk, n ≡ 1 by type). Bulge: fixed 4 at depth ≤ 2, free within 0.5–8 at depth ≥ 3 — a free n pinned at 8 is a note-level point-source/envelope identity question (cusp mimicry of an unfitted central point source, or envelope flux-grabbing; the repair menu maps the competing candidates — add(agn)+n re-warm / disk-role reapportionment / giant-elliptical waiver; raising the cap or freezing n=8 is never a repair). Bar: fixed 0.5, never released. Lens: free, prior < 0.5 — **n ≥ 0.6 hard** (not a lens profile; 0.5–0.6 note). OuterDisk (sersic variant): free, **< 1 hard**. Single Sersic: free (overall-concentration observable) |
| q (b/a) | Disk/edgedisk: free — oblique disk configurations are legal search directions (the expdisk template's default q/PA toggles `0` are NOT used in this workflow; always set `1`). Bulge: **q ≥ 0.3 hard** (0.3–0.4 watch-note). Bar: prior < 0.4, **hard cap 0.7** (0.5–0.7 watch-note). Lens: **> 0.5 hard**. Bar∧lens coexistence: **q_bar < q_lens (hard)**. Every shaped component: default bounds 0.05–1.0 |
...
| Flux shares | of the total model light: **bar 10–40% (note)**, **lens 5–35% (note)**, **f_lens < f_bar (hard — a lens must not outshine its bar)** |
| μ0 ordering | central surface brightness (analytic, from Mag/Re/n): **μ0_agn < μ0_bulge < μ0_disk (hard)**; **μ0_bar ≤ μ0_disk / 0.90 (hard — a bar may be only that much fainter than the disk at centre)** |
| Onion nesting | the 2·Re ellipses of the central chain outerdisk ⊃ disk ⊃ lens ⊃ bar ⊃ bulge must nest, in two layers (**hard**): (a) **area ordering** — the inner 2·Re ellipse area (Re²·q) must stay below the adjacent outer's, always on for any disk q; (b) **directional non-crossing** — skipped in the flat-disk regime (disk q < 0.3 or the edgedisk slot) only when corroborated by image-level evidence (Stage-1 edge-on/dust-lane classification, or consistency with the outer-isophote anchor q_iso measured once at beam_init: \|q_disk − q_iso\| ≤ 0.15); a fitted q < 0.3 that contradicts the anchor fires `disk_shape_inconsistency` (**note-level, advisory only** — q_iso is itself a coarse single-m outer-isophote measurement; suspected disk-role collapse / flux usurpation) and re-arms the directional layer. Per-pair exempt (both layers) when the inner is a bulge rounder than the outer by > 0.25 — a round bulge sits laterally wider than a flat bar, the normal configuration |
...
- **No candidate-declared bounds** (KILOGAS_120 defect follow-up: a lens pinned at its candidate-declared [10,20]/[0.55,0.95] with BIC_eff 38900 was mech-vetoed, and the forced relaxation candidates crowded the queue): candidates declare **initial values only**; any `cons_bounds` they carry is ignored by the transcription layer. Every band in every `iter{n}.cons` is therefore the default bound set — **original provenance, pins are note-level facts, never vetoes**. Guiding happens through warm-start initial values; misbehaviour is caught post-hoc by the mechanical checks (re_chain, degeneracy, onion, shape/profile priors).
- Standing exemptions: the q upper bound (1.0 is the domain edge), hard physical priors (Bar n = 0.5), and **Re pins at the mandatory Re floor** (the floor is not relaxable; the pin is a point-source identity question — note-level, expdisk rows compared in Rs).
...
- Temporary user constraints (if any): recorded verbatim in the working_note header (issue date + lifting condition) and carried in the `[Temporary constraints]` field of every `generate_galfit_beam_actions` `global_state_description` (mechanism and the currently active constraints: see `workflow_galfit.md` §Generation Spec); the companion exclusion issued 2026-09-04 was **REVOKED 2026-09-07** — companion candidates (`add(Companion, ...)`) are fully admissible again; galaxies run under the exclusion keep it in their graphs as provenance (do not retro-edit).
