...
| Single Sersic | `sersic` (n free), named `singlesersic` | exactly 1; sole legal co-tenant is the AGN point core (`{singlesersic, agn}` = the elliptical+point-core terminal state) | elliptical terminal state; if it fits q < 0.5, the classification must be revisited (a disk is implied). **Transitions**: single→multi — the first `add(<central component>)` (disk/bulge/bar/lens/outerdisk) MUST bundle the conversion `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68)` (the `disk` name is reserved for the expdisk slot; a multi-component model with a sersic `disk` or a `singlesersic` alongside anything but the AGN is invalid — `check_feedme_file` rejects it); `add(agn)` is exempt from the conversion (the AGN is not a central component; `{singlesersic, agn}` is legal without any conversion — the expert-typical elliptical+point-core answer, KILOGAS_221 retrospective); multi→single — remove-only transitions leaving one central component may revert `disk → singlesersic` (n free) |
