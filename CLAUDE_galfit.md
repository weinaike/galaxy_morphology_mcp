...
| Onion nesting | the 2·Re ellipses of the central chain outerdisk ⊃ disk ⊃ lens ⊃ bar ⊃ bulge must not cross (**hard**; skipped when disk q < 0.3 or the disk slot is edgedisk; per-pair exempt when the inner is a bulge rounder than the outer by > 0.25 — a round bulge sits laterally wider than a flat bar, the normal configuration) |
