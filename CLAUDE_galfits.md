...
5. **NEVER assume pixel scales** — always use `mcp__galmcp__re_arcsec2pix` to convert arcsec to pixels via FITS WCS headers. Do not use hardcoded values like 0.031"/px or 0.063"/px, as images may be drizzle-resampled at different scales. The reverse direction has `mcp__galmcp__re_pix2arcsec(re_pix, fits_file)`: whenever a VLM candidate reports a size (Re etc.) in pixels from a comparison panel, it MUST go through this tool (with the FITS of that panel's band) before being written into a `P*` five-tuple — a component's Re is a single band-agnostic arcsec value in the lyric, shared by all bands. Never paste pixel numbers into the five-tuple as arcsec.
