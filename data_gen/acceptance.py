...
def atomic_write_json(obj: dict, filepath: str):
    """原子写 JSON：先写 .tmp 再 os.replace，避免中断留下截断文件。"""
...
def save_trajectory(tree_data: dict, output_dir: str):
    ...
    采用原子写盘（.tmp + os.replace），保证一个存在的 trajectory 永远不是写到一半被杀留下的截断文件，
    从而「存在且带 metadata.completed 标记 = 真完成」这一续跑判据可信。
    ...
    # 原子落盘，确保中文/特殊字符不乱码，且有良好的缩进可读性
    try:
        atomic_write_json(tree_data, filepath)
