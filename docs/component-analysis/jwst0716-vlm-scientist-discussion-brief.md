...
```text
diffraction_psf 的最小定位要求：只有在 band、panel、region 和结构尺度均匹配时，diffraction_psf 才能否决局部数值证据
与逐波段 psf_veto 冲突时的优先级：VLM 的衍射判断与实际 PSF 模板方向检查冲突时，PSF模板方向检查优先。
与已分辨中心过量冲突时的处理：对已经通过数值分辨尺度门的中心过量，衍射标签应直接否决。
是否允许跨波段否决：只要有一个波段有bar或者中心源证据，就不应该被其他波段的衍射结构否决。
```
