...
from simulator_env.galfit_actions import apply_action_to_feedme, apply_unified_action_to_feedme, calculate_ssim, write_feedme_from_spec
...
class GalfitEnv(BaseSimulatorEnv):
    ...
    def _extract_metrics(self, summary_path: str) -> dict:
        ...
        包含: chi2_nu (2D约化卡方), chi2, ndof, 以及 MCP 计算的 1D 指标(bic1d/chisq1d_nu)
        注: BIC 直接引用 MCP main 分支(extract_summary_galfit)算出的 1D BIC，不自行计算。
        """
        ...
        try:
            with open(summary_path, 'r', encoding='utf-8') as f:
                ...
                # 4. 直接引用 MCP 计算的 1D BIC（summary 表格 "| BIC | <val> |"，即 bic1d）
                bic_match = re.search(r'\|\s*BIC\s*\|\s*([-+]?\d*\.?\d+(?:[eE][+-]?\d+)?)\s*\|', content)
                if bic_match:
                    metrics["bic"] = float(bic_match.group(1))

                # 5. 1D 约化卡方（MCP 计算）
                chisq1d_nu_match = re.search(r'χ²₁D/ν[^|]*\|\s*([-+]?\d*\.?\d+(?:[eE][+-]?\d+)?)\s*\|', content)
                if chisq1d_nu_match:
                    metrics["chisq1d_nu"] = float(chisq1d_nu_match.group(1))
    ...
    async def step(self, action: dict, current_feedme_path: str, current_png_path: str, output_dir: str, node_id: str, summary_path: str = None) -> dict:
        ...
        # 1. 物理动作执行：路由 方案B完整规格 > 统一决策 > 旧A/B/C/D
        if "spec" in action:
            success = write_feedme_from_spec(
                spec=action["spec"],
                root_feedme_path=current_feedme_path,
                new_feedme_path=new_feedme_path,
            )
        elif "structural" in action:
            success = apply_unified_action_to_feedme(
                decision=action,
