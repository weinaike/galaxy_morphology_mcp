...
_CHECK_REF_CUTOFF = "history context rounds are the current round or earlier"
_CHECK_MANUAL_SAMPLING = "manual sampling of state files (required before adjudication)"

_TS_PATTERN = re.compile(r"(\d{8}[T_]\d{6})")


def _round_timestamp(round_id: str) -> str | None:
    match = _TS_PATTERN.search(round_id)
...
def check_manifests(manifests: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for manifest in manifests:
        ...
        state_ts = _round_timestamp(manifest["state_round_id"])
        for entry in manifest.get("history_context", []):
            history_id = str(entry.get("round_id", ""))
            history_ts = _round_timestamp(history_id)
            if state_ts and history_ts and history_ts > state_ts:
                violations.append({"sample_id": sample_id, "check": _CHECK_REF_CUTOFF, "path": history_id, "detail": "history context references a round later than the state round"})
