...
2. **Axis-ratio hard limits (hard criterion; a hit is FAIL evidence)**:
   ...
   - **Bar q ≤ 0.6**: a fitted bar with b/a > 0.6 has lost its elongated bar identity (round-bar degeneration). A hit enters failed_checks as FAIL evidence (repair directions: re-initialise the bar q lower (q≈0.35), or the Bar+Lens split paths of the Lens trigger rules).
