...
5. 1D brightness curve and residuals
    ...
    - **Compact-source contamination of mid-radius 1D bumps (diagnostic knowledge)**: the 1D curve is an azimuthal average — a compact source at radius r (companion, bright knot) also produces a broad positive bump at that radius after azimuthal averaging, indistinguishable from an extended-component signature in 1D. When a mid-radius bump is found, distinguish the two morphologies on the 2D residual map: a **local compact bright blob** (azimuthal coverage ≲90°, compact-source signature) vs an **azimuthally continuous ring-like positive residual** (coverage ≳180°, extended low-n component signature). **On this branch neither morphology may be actioned** (companions and Lens are outside the action space — see the Restricted Action Space note in Phase 2); describe the bump's position, width, amplitude and 2D morphology as context only; such residuals are left unfitted.
    - **Lens-type bump diagnosis (diagnosis only; execute when the parent state already has a Bar or Bulge)**: check whether the 1D residual curve (Δμ = Data − Model) shows a **broad positive bump** about 1.5–2.5·Re_bar from the centre (about 2–4·Re_bulge when there is no Bar). This bump is the radial flux signature of a Lens-type (low-n extended) component, unlike spiral-arm residuals — spiral arms appear as spiral bands on the 2D residual map and are suppressed in 1D amplitude by azimuthal averaging, whereas a lens-type bump is near-circularly symmetric in 2D and significant in 1D. When such a broad bump is found, record in the feature description its position, width and amplitude, **tagged as a lens-type feature — outside this branch's action space (left unfitted; do NOT generate an add(Lens) candidate)**. **Precondition**: if the Disk outer-flux-deficit check hit, the bump reading may be contaminated by a too-small disk Re — state the relative contributions of both.
    - **Quantitative measurement of 1D residual bumps (mandatory; feeds the Phase-2 Re triplet)**: for every significant bump or systematic offset on the 1D residual curve, besides the qualitative description you **must** report its **peak radius, span and amplitude** (px / mag) — Re_init ≈ bump peak radius / 2 for a missing component (see the 🔑 Re Convention's residual-geometry conversion); the Re triplets of Bar candidates are derived directly from it.
...
Using the 2·Re component ellipses on the Model panel (colour-attributed: disk=blue, bulge=green, bar=orange, lens=red), the legend parameters (one line per component: `name Mag Re(px) n q PA°`), the parameter summary above, the Phase-1 visual features, and the bound-hit reports in local_state_description, check the **main-galaxy central components** (disk/bulge/bar; the AGN (psf component) does not take part in the physicality verdict) item by item:

1. **Nested containment ("onion" structure; the core check)**: on the Model panel, read each component's 2·Re ellipse by colour and check the chain `disk ⊃ bar ⊃ bulge` (for the components that actually exist, take **adjacent pairs** along the chain; drop the missing ones from the chain):
   ...
   - **Area ratio (soft signal; note only, not a FAIL)**: the inner ellipse's contour-area fraction within the adjacent pair (healthy decompositions are typically ≲ 1/4) is observational reference only; two ellipses of similar size ("evenly matched", fraction ≳ 1/2), or even an inner component brighter than the outer one (Mag overtaking, e.g. a legal layering of a bright bar + faint extended disk), **do not constitute a FAIL as long as nested containment holds** — record them in failed_checks with a `[note]` prefix for the orchestrator. Area signals count toward FAIL only when accompanied by one of the structural violations (a)/(b)/(c).
...
2. **Axis-ratio hard limits (hard criterion; a hit is FAIL evidence)**:
   - **Bulge q ≥ 0.5**: a fitted bulge with b/a < 0.5 is physically inconsistent with a bulge — a genuinely flattened central component is bar territory, not a bulge. A hit enters failed_checks as FAIL evidence (repair direction: the Flat-Bulge → Bar Candidate Trigger Rule).
   - **Bar q ≤ 0.6**: a fitted bar with b/a > 0.6 has lost its elongated bar identity (round-bar degeneration). A hit enters failed_checks as FAIL evidence (repair directions: tighten the bar q upper bound, or re-examine the component's identity per the Flat-Bulge → Bar Candidate Trigger Rule).
3. **Shape priors (soft signal; note only, not a FAIL)**: conflicts between n and role (e.g. a bulge n drifting toward 0.5 degenerate with a bar, disk n ≠ 1) etc. are all tagged `[note]` and do not constitute a FAIL — shape priors are heuristic references, not structural violations. (The bulge/bar q limits of check 2 are **hard** criteria and are not covered by this soft clause.)
...
7. **Outermost component out of bounds (outer edge exceeding the fitting region)**: hit if the 2·Re ellipse of the outermost main-galaxy component (usually the disk) **extends beyond the model-panel boundary in any part** (the panel range = the fitting region). Two layers of physical meaning: (a) **unconstrained by data** — the fitting region was chosen to "enclose the whole galaxy"; if a component's 2·Re already leaves the region, a substantial fraction (~half) of its flux falls outside the data window, its parameters are set by extrapolation rather than data, and the converged values are untrustworthy; (b) **flux-mismatch degeneracy** — the usual cause is an inner component (bulge/bar) over-absorbing flux that belongs to the outer component, or an inner component inflating and squeezing the outer component's luminosity share, forcing the fitter to grow the outermost component's Re and spread out at lower surface brightness to cover the mid/outer-radius data flux ("inner grabs, outer compensates"). This signal shares its origin with the degeneracy families in the Disk Outer-Flux-Deficit Trigger Rule; repair candidates should be generated against that rule (and the 🔑 Bound-Relaxation Rule) first.
...
- **swap_hint rule**: write swap_hint = `disk_bulge_swap` only when verdict=FAIL **and** the only checks hit are the {disk, bulge} nesting inversion (the bulge ellipse larger than or poking out of the disk ellipse — two free Sersics swapped): swapping the labels of two free Sersics is the standard fix, and a repair candidate in the label-swap direction should be given. Inversions involving bar are **strictly forbidden to swap** (strong physical priors, not interchangeable); write swap_hint = none.
- **PASS threshold (judge by the onion structure, leniently)**: the hard standards are (i) the **concentrically nested onion structure** — all main-galaxy ellipses share a centre, are wrapped layer by layer as `disk ⊃ bar ⊃ bulge`, each inner layer fully contained in the outer (major-axis tips included), no inversion, no crossing, no identity degeneracy, and the outermost component does not leave the fitting region — and (ii) the **axis-ratio hard limits** of check 2 (bulge q ≥ 0.5, bar q ≤ 0.6). PASS only if no hard criterion is hit. Similar areas ("evenly matched"), a brighter inner component (Mag overtaking), shape-prior deviations (out-of-range n, etc.) and other **soft signals do not constitute a FAIL** — record them in failed_checks with the `[note]` prefix for the orchestrator (write "none" if there are no notes). In borderline cases (a bar-ellipse tip just touching the disk-ellipse boundary, a disk 2·Re exactly tangent to the panel edge without exceeding it) **lean toward PASS** unless there is clear evidence of a structural violation — a note is better than a FAIL.
...
Entries in `[Verified basins]` (e.g. "bar PA ≈ 45°, anchored successfully in two rounds") are **parameter ranges validated by fitting**. Absolute pixel readings naturally carry ±10px noise, whereas verified basins survived χ²/BIC tests — a basin outranks a single-round visual reading.
...
**It is forbidden** to bundle unrelated atomic operations (e.g. adding a Bulge while changing the Disk PA while deleting a bar).
...
- **Priority relations with the dedicated trigger rules (fool-proof clause)**: this rule is the default outlet for all bound-hit parameters; where a dedicated rule covers a parameter with a closed enumeration of competing paths, it **must** keep a path open for "relax the self-imposed bound" or explicitly declare it inapplicable — a closed enumeration must not silently suppress this rule (real incident: a component's Re hit a self-imposed cap with no degeneracy, yet a closed enumeration kept the "relax" candidate from appearing for many consecutive rounds).
...
- **Scope and AGN exemption**: this convention (triplet requirement included) applies to all components **with a physical Re quantity** — Bulge / Bar / Disk and other sersic/expdisk components. **The AGN uses the `psf` component type and has no Re parameter**: `add(AGN, ...)` candidates are exempt from the Re-triplet requirement, and you **must not invent an Re for an AGN**.
- **add() completeness (hard requirement)**: whenever the target type of an `add(<Type>)` has a physical Re quantity, the candidate's primitives **must** explicitly include the `[Re_min, Re_init, Re_max]` px triplet (noting the measurement-source panel). **An add() candidate without Re is invalid** — Re is the allocator of radial flux budget, and the orchestrator cannot invent a physical Re initial value for you; omitting it forces the orchestrator to guess, possibly triggering chain degeneracies in neighbouring components (see above). This applies equally to Bulge / Bar, regardless of component type or depth (AGN alone is exempt).
- **Degradation clause when measurement is impossible (silent omission forbidden)**: if the target feature is too diffuse or contaminated and its radial extent genuinely cannot be measured reliably, **do not silently omit Re** — instead set a deliberately wider triplet from the Re total order (`Re_disk > Re_bar > Re_bulge`; drop missing components and take the adjacent-pair interval) and **explicitly state** in physical_motivation "Re was not measured from the residuals; taken from the total-order prior interval [a, b] px". Taking a prior ratio (e.g. "0.3–0.6×Re_disk") in place of a measurement remains forbidden in the normal case (see above); the prior interval is an explicit fallback for failed measurement, not a routine option.
...
Whenever the target type of an `add(<Type>)` is a component with shape parameters (Bulge / Bar / Disk), the primitives must, besides the Re triplet, **explicitly** give:

1. **n initial value + free/fixed state** (e.g. `n=2.5 free`, `n=0.5 fixed`) — bare `n_free` without a value is forbidden. Physical priors: bulge 2–4, disk 1, bar 0.5; the initial n affects basin choice (real incident: released from 4.0 it converged to 1.8, from 3.0 it converged to 3.0 — the same free parameter, two basins);
...
Before generating candidates, explicitly enumerate the legal component-type space of the main galaxy. This is a "menu", not "the answer" — actual candidates must still be based on Phase-1 residual evidence and the recognition rules below.

### 🔒 Restricted Action Space (branch-level hard rule)

This branch restricts the solution space to a fixed component inventory: **Disk (`expdisk`), Bulge (`sersic`), Bar (`sersic` with n=0.5 fixed), AGN (`psf`)** — plus the **Single Sersic** start/terminal state, the **F1** Fourier decoration and the fixed **sky**. The following components are **outside the action space**: **Lens, OuterDisk (envelope / 2nd expdisk), Edge-on Disk (`edgedisk`), Companion**.

- Generating any candidate that adds, tunes, type-switches or otherwise maintains one of these four components — `add(Lens)`, `add(Companion)`, `add(OuterDisk)`, `tune(Disk, →edgedisk)`, `tune(companion, ...)`, a Bar+Lens split, a companion sersic↔psf switch, … — is **forbidden**: such candidates are invalid and the orchestrator discards them outright.
- **Diagnostic knowledge stays**: Phase-1 observations of lens-type ring bumps, companion-type bright blobs or unfitted outer envelopes remain valuable context — describe them in the feature description tagged "outside this branch's action space (left unfitted)", but **never convert them into actions**. Higher-order residual features of these kinds may legitimately be left unfitted.
- An edge-on-looking galaxy is fitted with the ordinary `expdisk` Disk with free q (oblique/flat disk configurations are legal directions on this branch); no `edgedisk` switch exists.

### A. Main-galaxy component types
The final component set of the main galaxy belongs to one of:
...
| AGN | `psf` component (no physical Re) | enabled only when the Bulge Re collapses below 0.2 px (a psf competing variant may be built in the 0.2–0.5px border zone) |

**🔑 Closed component alphabet (hard rule)**: the table above (plus the F1 mode, the sky and the `singlesersic` start) is the **complete** list of legal components on this branch — an `add()` target outside it (Lens / OuterDisk / edgedisk / Companion included) is **invalid** and the orchestrator discards the candidate outright. In particular the **AGN (`psf`) is the sole central point-source component**: any candidate that adds a central point source / compact core under a different name or type must be rewritten as an `add(AGN, psf)` candidate satisfying the admission rule above (Bulge Re collapsed < 0.2 px mandatory replacement; 0.2–0.5 px border-zone competing variant), or dropped.

**Multiplicity rules (hard; per the CLAUDE.md solution-space definition)**: at most **one each** of Disk, Bulge, Bar and AGN; **one** F1 at most, only on the Disk (or the single Sersic when no Disk exists); exactly one sky (fixed after the first fit). **Naming**: the single-component start is named `singlesersic` (sersic with **free** n — the elliptical terminal state, excluding all other luminous components); the `disk` name is reserved for the **expdisk** slot of multi-component models — a multi-component model containing a sersic named `disk`, or a lingering `singlesersic` alongside another component, is an **invalid state**.
...
- n: Bar 0.5 fixed; Bulge fixed 4 at depth≤2 or free 0.5–8 at depth≥3; the Disk is expdisk (no n);
- q: priors Bar < 0.4, Bulge > 0.5; Disk q and PA are **free** parameters (oblique disk configurations are legal directions; the expdisk template's default q/PA toggles 0 are not used in this workflow); every shaped component bounded 0.05–1.0;
...
**Trigger**: whenever the parent state's `expected_C'` contains **≥ 2 main-galaxy central components** (any two or more of Disk/Bulge/Bar), the concentric constraint **must** be in effect — a default hard constraint, not an on-demand option, independent of the candidate's direction (add/tune/fix constraints).
...
**VLM duty**: when generating candidates that add main-galaxy central components (`add(Bulge)` / `add(Bar)` / `add(AGN)` etc.), `physical_motivation` **must explicitly mention** "the new component is bound to the same centre as the existing Disk/Bulge/Bar via the `.cons` x,y offset"; you may use `concentric_bound` in `expected_behavior_tag`. For `tune(...)` or `remove(...)` candidates, as long as `expected_C'` still contains ≥ 2 main-galaxy central components, the concentric constraint is inherited by default (no need to restate every round, but never forget it).

### C. Companions — outside this branch's action space

Companions (independent Sersic/PSF component blocks, outer or embedded) are **not part of the solution space on this branch** — see the 🔒 Restricted Action Space note above. Observe and describe companion-type residual blobs in Phase 1 as context (tagged "outside this branch's action space"), but never generate `add(Companion)` / `tune(companion, ...)` / companion type-switch candidates; the orchestrator discards any that slip through.
...
- **Distinguishing from a mid-radius bump**: a lens-type bump peaks at ~1.5–2.5·Re_bar (between the centre and the disk); the disk outer-flux-deficit positive residual extends outward from r > 2×Re_disk. Both can coexist — if the disk Re is too small, the mid-radius bump reading is contaminated (lens-type features are outside this branch's action space and stay unfitted; the `tune(disk, larger Re)` candidate remains the actionable output).
...
- **Priority over the central-component rules**: if this round triggers both the disk outer-flux deficit and rules such as flat-Bulge→Bar, the `tune(disk, larger Re)` candidate **must** occupy a slot in the candidate list (it must not be squeezed out by other rules' candidates).
...
- **Role adjudication (key)**: in galfit the "Disk component" (expdisk) and the "single Sersic" (sersic) are different component types, distinguishable by the `0)` line —
    - **Disk component** (the disk member of a multi-component decomposition, coexisting with Bulge/Bar) → `expdisk` (its `4)` row is the scale length Rs; the Re triplet you give refers to the effective radius Re=1.68·Rs, converted by the orchestrator when writing).
...
- **Edge-on note**: there is **no** `edgedisk` switch on this branch — a galaxy with edge-on signatures (b/a < 0.17 with a dust lane / disk thickness) is fitted with the ordinary `expdisk` Disk with free q (oblique/flat disk configurations are legal directions; see the 🔒 Restricted Action Space note).

### depth = 2 (the two-component foundation is established)
Output **2–3** candidates. Typical directions: fix constraints, release/tighten a parameter, add a compact point-source component (an AGN, `psf` — only under the Bulge-Re-collapse admission rule), adjust a component's initial shape/PA.
...
- **Follow the component-addition order**: `Disk → (F1 if lopsidedness detected) → Bulge → Bar → AGN (psf, under the Bulge-Re-collapse admission rule)`. Bar/AGN recognition must follow the `<Overall workflow of galaxy component analysis>` section. Companion/lens/envelope features observed in Phase 1 stay unfitted on this branch (outside the action space).
...
- **Directional diversity** (with multiple candidates): candidates must span **clearly different** directions. Typical contrasting pairs:
    ...
    - "fix constraints" vs "adjust initial shape/attitude" (e.g. fix the bulge↔disk concentricity vs tune(disk, q_init, PA_init) into an oblique configuration)
    - "Occam" vs "deepen" (e.g. remove(agn) vs tighten the bulge_Re cap)
    - "conversion" vs "addition" (when the Flat-Bulge trigger holds: tune(Bulge→Bar) vs add(Bar)+tune(Bulge, q_min=0.7))
    - "grow the skeleton" vs "repair the skeleton" (e.g. tune(disk, larger Re) vs tighten the bar Re cap when the parent Bar's Re/q is anomalous)

## Required candidate fields
Every candidate must give:
- **expected_C'**: the expected component inventory after applying the action (e.g. `{Disk, Bulge, Agn}`)
- **expected_behavior_tag**: expected fit-behaviour tag (short snake_case, e.g. `bulge_n_free_release`, `agn_add_psf`, `bar_pa_correct`, `bar_add_quadrupole`, `constrain_fix_concentric`, `occam_remove_agn`, `disk_add_fourier_f1`)
...
- **Physicality-verdict review (hard requirement, first item)**: the output starts with a properly formatted `## Physicality Verdict` block (all three fields verdict / failed_checks / swap_hint present), and the nested-containment check (check 1), the axis-ratio hard limits (check 2: bulge q ≥ 0.5, bar q ≤ 0.6), the globally-unblemished check (check 6) and the outermost-component out-of-bounds check (check 7) really were performed pair by pair against the ellipses, the legend q values and the panel boundary. On verdict=FAIL, confirm that at least one candidate this round repairs failed_checks (with swap_hint=disk_bulge_swap, include a disk ↔ bulge label-swap candidate); a verdict=PASS that contradicts Phase-1 notes of "Re total-order inversion", a bulge q < 0.5 / bar q > 0.6 legend value, or described ellipse crossing/poking/offset/out-of-panel features is self-contradictory — fix one of the two.
- **add() Re completeness (hard requirement, item by item)**: for every `add(<Type>)` primitive — whenever the target type **has a physical Re quantity** (Bulge / Bar / Disk and other sersic/expdisk components), the primitives **must** explicitly include the `[Re_min, Re_init, Re_max]` px triplet (with the measurement-source panel). An add() candidate missing Re is **invalid** and must be rewritten after returning to Phase 1 for the radial measurement (see the add()-completeness clause of the 🔑 Re Convention). **`add(AGN)` (psf component, no Re quantity) is exempt — you must not invent an Re for an AGN.** If a prior interval from the Re total order was used because the feature could not be measured, this must already be declared in physical_motivation (see the degradation clause); otherwise it is equally a violation.
- **Re-triplet adjacency review (hard requirement, item by item)**: for every add() candidate carrying an Re triplet, insert the new component at its proper place in the total order `Re_disk > Re_bar > Re_bulge` and check the adjacent inner/outer components against the Model-panel legend's converged `Re(px)` — `Re_min > adjacent inner Re` and `Re_max < adjacent outer Re` must both hold (see the total-order adjacency clause of the 🔑 Re Convention). A candidate violating adjacency (e.g. bar Re_min < bulge Re, bar Re_max > disk Re) is equally **invalid** and must rewrite the triplet per the truncation rule.
...
- **Closed-alphabet review (hard requirement)**: every `add()` target this round belongs to the Candidate Space Alphabet's legal types (Disk/Bulge/Bar/AGN, plus the `singlesersic` start and the F1 mode) — Lens, OuterDisk, edgedisk and Companion are **outside this branch's action space** and any candidate targeting them must be deleted. The AGN (`psf`) is the **sole** central point-source component and may be added only under the Bulge-Re-collapse admission rule. Any candidate adding a central point source / compact core under any other name or type must be rewritten as a compliant `add(AGN, psf)` candidate or deleted.
- **Quadrupole alternative-decomposition review (hard requirement)**: if Phase 1 described quadrupole/butterfly residuals and the bar direction has been refuted in `[Refuted hypotheses]` (or the parent shows long-standing bulge-q lower-bound hits + repeated bar failures), this output must include at least one disk oblique-ellipticity candidate (see Competing decompositions after the bar hypothesis fails); otherwise the waiver must be stated explicitly in a Candidate's physical_motivation — silent skipping is forbidden.
...
- **Generating Disk type/n-release candidates is forbidden** (the Disk is always expdisk; n≡1 is guaranteed by the component type, see the Disk-n operating rules); a single-Sersic model (no parallel Bulge/Bar) with free n is the exception
...
- **Concentric-constraint review (hard requirement)**: every candidate whose `expected_C'` contains **≥ 2 main-galaxy central components** (Disk/Bulge/Bar) **must** inherit the concentric constraint by default — `add(...)` candidates' `physical_motivation` must explicitly mention "bound to the same centre via the `.cons` x,y offset"; `tune(...)` / `remove(...)` candidates inherit silently without restating. If `add(Bulge/Bar/AGN)` candidates appear this round without any candidate's physical_motivation mentioning concentric binding, this is a violation.
