...
3. 2D residual map — central region (within the central galaxy's extended area):
    ...
    - **Radial measurement of new-component features (mandatory; feeds the Phase-2 Re initial value)**: for every suspected new-component feature identified in the central or extended region (bar-like/quadrupole, compact core, ring-like bright band, isolated bright blob), besides the qualitative description you **must** report its **inner and outer radii** (px, noting which panel the measurement came from) — the `Re_init` of Phase-2 `add(...)` candidates is derived directly from these measurements (see the 🔑 Re Convention's residual-geometry conversion and the Bar special clause); without this measurement the candidate cannot carry a valid Re_init and is invalid.
...
5. 1D brightness curve and residuals
    ...
    - **Quantitative measurement of 1D residual bumps (mandatory; feeds the Phase-2 Re initial value)**: for every significant bump or systematic offset on the 1D residual curve, besides the qualitative description you **must** report its **peak radius, span and amplitude** (px / mag) — Re_init ≈ bump peak radius / 2 for a missing component (see the 🔑 Re Convention's residual-geometry conversion); the Re initial values of Lens / Bar / Companion candidates are derived directly from it.
...
- **PASS threshold (judge by the onion structure, leniently)**: the hard standards are (i) the **concentrically nested onion structure** — all main-galaxy ellipses share a centre, are wrapped layer by layer as `disk ⊃ lens ⊃ bar ⊃ bulge`, each inner layer fully contained in the outer (major-axis tips included), no inversion, no crossing, no identity degeneracy, and the outermost component does not leave the fitting region — and (ii) the **axis-ratio hard limits** of check 2 (bulge q ≥ 0.3, bar q ≤ 0.7). PASS only if no hard criterion is hit. Similar areas ("evenly matched"), a brighter inner component (Mag overtaking), shape-prior deviations (out-of-range n, a too-flat lens, etc.) and other **soft signals do not constitute a FAIL** — record them in failed_checks with the `[note]` prefix for the orchestrator (write "none" if there are no notes). In borderline cases (a bar-ellipse tip just touching the lens-ellipse boundary, a disk 2·Re exactly tangent to the panel edge without exceeding it) **lean toward PASS** unless there is clear evidence of a structural violation — a note is better than a FAIL.
...
- No hit → a legal candidate (Occam remove(AGN), cleaning up a collapsed component, remove+add replacement combos, etc.); generate normally, with the novelty_claim citing the ledger to state "the post-removal state is not in the ledger".
...
- `add(type, initial_params)` — add a component (e.g. `add(Bulge, n=2.5 free, q=0.85, PA=135, Re_init=2px)`; whenever the target type has a physical Re quantity, the key params **must** include the px initial value Re_init and the shape-minimal set n/q/PA — AGN excepted, see the 🔑 Re Convention and the 🔑 Shape-Minimal-Set Convention)
...
- **Explicit px units**: always write Re as a pixel value with the `px` suffix (e.g. `Re_init=12px`), noting which panel the measurement came from (original / residual / 1D curve). **Never** bare numbers, **never** arcseconds — you are looking at a pixel grid; px values are **the same unit** as the feedme `4)` parameter row, and the orchestrator writes them verbatim with no conversion whatsoever (the expdisk Rs conversion Re=1.68·Rs is handled by the orchestrator; the Re values you give always refer to the effective radius Re).
- **Always give the initial value `Re_init`** (NO min/max bands — candidates declare initial values only; any declared band is ignored by the transcription layer), grounded in the **measured radial extent of that component's candidate region in the residual map**, not in prior ratios (e.g. "0.3–0.5×Re_disk"). Re is the allocator of a component's radial flux budget: an initial value off by a factor of two puts that component's light into the wrong annulus and triggers chain degeneracies in its neighbours — of all parameters it has the largest effect on the fit and must be pinned precisely from evidence.
...
- **Total-order adjacency constraint (two-evidence Re setting, hard requirement)**: Re_init must not be set from that component's own residual feature alone — it must also respect the fitted Re of the **adjacent inner/outer components** in the total order `Re_disk > Re_lens > Re_bar > Re_bulge` (read the converged `Re(px)` from the Model-panel legend, directly comparable in the same panel coordinates, no conversion): after inserting the new component at its proper place in the chain, let the adjacent inner component's Re be R_in and the adjacent outer component's Re be R_out; `R_in < Re_init < R_out` **must** hold. When the two evidences conflict (the measured Re_init falls outside the adjacency interval), shift Re_init into the interval and state in physical_motivation "the measured residual value conflicted with the total-order adjacency constraint and was moved to X px". An initial value invading a neighbour's Re interval invites identity degeneracy and Re-inversion degeneration.
  - **Counter-example (a real incident; learn from it)**: with a parent bulge Re=3.5px and disk Re=23px, a new bar was initialised at Re=4px close below the bulge — the bar converged inside the bulge; the result was Re inversion (the bar ellipse nested inside the bulge) + bar q hitting its lower bound, a physicality FAIL, wasting two rounds of budget. The correct move: the adjacency constraint requires `3.5px < Re_init < 23px`, e.g. `Re_init=6px` (still centred on the quadrupole/bar-feature measurement).
- **Bar special clause**: the bar sits mid-chain (bulge < bar < disk); its Re decides how much flux it takes from the bulge side and the disk side, and an error of a factor of two triggers chain role-swaps on both sides (the bar squeezing the bulge, or inflating to swallow the disk). Before generating any add(Bar) / tune(bar, Re) candidate, you must first bound the bar-like/quadrupole feature region's **inner and outer radii** in the residual map, set Re_init from them, and satisfy the total-order adjacency constraint (`Re_bulge < Re_init < Re_disk`; see above).
...
- **Scope and AGN exemption**: this convention (Re_init requirement included) applies to all components **with a physical Re quantity** — Bulge / Bar / Lens / Companion / Disk and other sersic/expdisk components. **The AGN uses the `psf` component type and has no Re parameter**: `add(AGN, ...)` candidates are exempt from the Re_init requirement, and you **must not invent an Re for an AGN**.
- **add() completeness (hard requirement)**: whenever the target type of an `add(<Type>)` has a physical Re quantity, the candidate's primitives **must** explicitly include the `Re_init` px value (noting the measurement-source panel). **An add() candidate without Re is invalid** — Re is the allocator of radial flux budget, and the orchestrator cannot invent a physical Re initial value for you; omitting it forces the orchestrator to guess, possibly triggering chain degeneracies in neighbouring components (see above). This applies equally to Bulge / Bar / Lens / Companion, regardless of component type or depth (AGN alone is exempt).
- **Degradation clause when measurement is impossible (silent omission forbidden)**: if the target feature is too diffuse or contaminated and its radial extent genuinely cannot be measured reliably, **do not silently omit Re** — instead take Re_init from the Re total order (`Re_disk > Re_lens > Re_bar > Re_bulge`; drop missing components and take the midpoint of the adjacent-pair interval) and **explicitly state** in physical_motivation "Re was not measured from the residuals; taken from the total-order prior interval [a, b] px". Taking a prior ratio (e.g. "0.3–0.6×Re_disk") in place of a measurement remains forbidden in the normal case (see above); the prior interval is an explicit fallback for failed measurement, not a routine option.
...
Whenever the target type of an `add(<Type>)` is a component with shape parameters (Bulge / Bar / Lens / Companion / Disk), the primitives must, besides Re_init, **explicitly** give:
...
| AGN | `psf` component (no physical Re) | enabled only when the Bulge Re collapses below 0.2 px (a psf competing variant may be built in the 0.2–0.5px border zone), or — with a healthy non-collapsed bulge present — an obvious positive-residual leftover within 0–5 px of the 1D profile that the Bulge cannot absorb, or — on a SingleSersic-only parent (elliptical regime, no bulge exists) — a compact central positive residual / 1D-profile spike within <5 px survives an otherwise clean single-Sérsic fit (outskirts noise, q > 0.5) |

**🔑 Closed component alphabet (hard rule)**: the table above (plus companions, the F1 mode, the sky and the `singlesersic` start) is the **complete** list of legal components in this workflow — an `add()` target outside it is **invalid** and the orchestrator discards the candidate outright. In particular the **AGN (`psf`) is the sole central point-source component**: any candidate that adds a central point source / compact core under a different name or type must be rewritten as an `add(AGN, psf)` candidate satisfying the admission rule above (Bulge Re collapsed < 0.2 px mandatory replacement; 0.2–0.5 px border-zone competing variant; the bulge-present 0–5 px un-absorbable leftover; on a SingleSersic-only parent, the central-spike branch), or dropped.

**Multiplicity rules (hard; per the CLAUDE.md solution-space definition)**: at most **one each** of Disk/edgedisk (mutually exclusive; edgedisk replaces the Disk and takes over its slot in the concentric anchor and the Re chain), Bulge, Bar, Lens, OuterDisk and AGN; **one** F1 at most, only on the Disk/edgedisk (or the single Sersic when no Disk exists); companions may be multiple (`companion`, `companion2`, …); exactly one sky (fixed after the first fit). **Naming**: the single-component start is named `singlesersic` (sersic with **free** n — the elliptical terminal state; its **sole** legal co-tenant is the AGN point core: `{singlesersic, agn}` is the elliptical+point-core terminal state, requiring NO SingleSersic→Disk conversion); the `disk` name is reserved for the **expdisk** slot of multi-component models — a multi-component model containing a sersic named `disk`, or a `singlesersic` alongside any component other than the AGN, is an **invalid state**.

**SingleSersic → Disk conversion (mandatory bundling)**: any `add(<central component>)` candidate whose parent is a SingleSersic state (the model's only luminous component is `singlesersic`) **must bundle** the conversion `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68)` as its second primitive — the two operations are semantically cohesive (building the disk skeleton) and fit within the ≤2-primitive cap. Without the conversion the child contains a sersic disk with free n — an invalid state the orchestrator rejects before fitting (`check_feedme_file` error). **Exception — `add(agn)`**: the AGN (psf) is NOT a central component for this rule; `add(agn, psf, x/y = core px, mag ≈ small-aperture)` on a SingleSersic parent is legal WITHOUT any conversion ({singlesersic, agn} is the elliptical+point-core terminal state — do not force a disk skeleton on a galaxy whose single-Sérsic residuals are basically clean except a compact central spike). Conversely, a remove-only transition leaving exactly one central component may revert `disk → singlesersic` (sersic, n free; the elliptical terminal state).

**Hard parameter limits (candidates outside these are invalid)**:
- Re total order `re_disk > re_lens > re_bar > re_bulge` (existing central components only, decrease outward with a 3% relative tolerance — an inner Re may exceed the adjacent outer Re by at most 3% (e.g. `re_bar ≤ 1.03·re_lens`); OuterDisk sits above re_disk; edgedisk plays the Disk slot);
...
| R ≥ 2.3 | `sersic` | resolved companion galaxy: n free (typical 0.5–4, default bounds), q from the visual ellipticity, PA from the visual orientation (N=+Y), Re_init from the measured extent (≈ half the visible major extent) |
...
1. **`tune(Bulge→Bar, n=0.5 fixed)`** (conversion) — tests "this flat component is itself a Bar; there is no independent Bulge". The component count is unchanged; Re inherits the bulge converged value via warm start (`tune` adds no component, so no Re_init needed). Decision: ΔBIC < 0 supports it.
2. **`add(Bar, n=0.5 fixed, q<0.4, PA≈bulge PA, Re_init=from the inner/outer radii of the residual quadrupole/bar-like feature) + tune(Bulge, q≈0.7)`** (addition + rounding) — tests "beyond the flat Bulge there is also an independent Bar". Component count +1. Decision: ΔBIC < −10 is required to be significant (clearing the added-parameter penalty). Rounding the Bulge breaks the Bar/Bulge degeneracy so the fitter does not push either to extreme parameters. **The added Bar must carry Re_init** (measurement per the Bar special clause of the 🔑 Re Convention).
...
## Flux-Misallocation Trigger Rule (VLM must read; generates `tune(component, mag_init)` candidates)

**Motivation**: mag is a free parameter every round and warm-started from the parent converged values — the optimiser normally reallocates flux by itself, which is why mag-tune candidates were historically never proposed. This rule covers the **basin-trap cases where it cannot**: the flux allocation is stuck in a bad split and no structural action is warranted. Flux reallocation is explicitly part of the search space (the solution space's Mag row: "no default bounds; flux reallocation is part of the search").

### Trigger (any one; the numeric signals arrive in local_state_description under "[orchestrator numeric-rule delegation] Flux-misallocation signals")

| # | Trigger (numeric part from the orchestrator; residual part from your Phase 1) | Meaning |
|---|---|---|
| **A — flux-evidence contradiction** | a main-galaxy component's ΔMag vs the disk > 3 (flux < ~6% of the disk) **AND** Phase 1 still shows significant residual features (≳ ±0.1 mag) within that component's radial zone | the component has been starved of flux its zone still needs — faintness and residual evidence contradict each other |
| **B — chronic flux bleed** | a component's fitted Mag has drifted monotonically by ≥ 0.3 mag/round over ≥ 2 consecutive rounds (trajectory quoted in local_state_description) | the optimiser is slowly abandoning the component — a bad allocation basin (complementary to the frozen-parameter [sub-converged] signature) |
| **D — sub-convergence repair** | this round was flagged [sub-converged] and local_state_description lists mag initial-value reapportionment among the re-run prescriptions | breaking the frozen allocation is part of the corrected re-run |

### Action

`tune(component, mag_init = <new initial magnitude>)` — the **only** edit is the component's `3)` row initial value:

- mag stays **free** (toggle 1) after the move. Do **NOT** write `.cons` mag bands: a relative mag band would constrain the optimiser's free reallocation (against "flux reallocation is part of the search") and bare-number bands re-risk the input-relative encoding trap. Initial-value moves only.
- Size the delta from the residual evidence, not from thin air: a ±0.1 mag systematic residual across the component's zone ≈ reapportion ~0.5–1 mag; keep the new init within ±1.5 mag of the parent converged value (this is a **basin escape**, not a re-invention — order-of-magnitude jumps violate the Class-B fill range).
- May bundle with at most one cohesively-related primitive of the same repair goal (e.g. the band re-encode of the same [sub-converged] re-run); never with unrelated structural changes.

### Priority and waiver

- **Normal six-dimension scoring; no mandatory-retention floor** — mag misallocation is second-order (the optimiser usually self-heals), so these candidates compete on their merits like any other.
- But when a trigger holds, this candidate (or an **explicit waiver** in physical_motivation citing why the flux evidence does not support reallocation, e.g. "the component's faintness matches a genuine [zombie]; its zone's residual belongs to the neighbouring component") is **REQUIRED** — silent skipping is forbidden.
...
- **No companion yet (or none co-located with the bump) but the 2D residual at the same radius is a local compact blob** (stronger still if the original image shows a point/extended source there): this rule's priority output is an `add(Companion, x_blob, y_blob, compact prior Re_init≈PSF scale to a few px (from the blob's measured radius), free centre (the coordinate initial value is the pixel coordinate))` candidate — explain the bump at that radius with a compact source rather than `add(Lens)`. Write the coordinates as pixel values directly; the orchestrator fills them verbatim into the feedme. The embedded-companion timing constraint is automatically satisfied here: Path B presupposes the parent state already has a Bar or Bulge.
...
- **profile**: `sersic`
- **n**: free (toggle=1), initial ~0.3 (prior n<0.5)
- **Re**: free, initial value half the 1D bump peak radius per the 🔑 Re Convention; Re_init must sit inside the total order (> Re_bar or Re_bulge, whichever larger; < Re_disk) — no min/max bands (initial value only)
- **q (axrat)**: free, initial ~0.8 (Lens near-round, q>0.5)
...
- **Role adjudication (key)**: in galfit the "Disk component" (expdisk) and the "single Sersic" (sersic) are different component types, distinguishable by the `0)` line —
    - **Disk component** (the disk member of a multi-component decomposition, coexisting with Bulge/Bar/Lens) → `expdisk` (its `4)` row is the scale length Rs; the Re value you give refers to the effective radius Re=1.68·Rs, converted by the orchestrator when writing).
...
- **lopsidedness not detected + bar detected** → **1–2 candidates**: `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68) + add(Bulge, n=4 fixed, Re_init=from the first round's central-core radial measurement)` (the standard Disk+Bulge split, with the **mandatory SingleSersic→Disk conversion bundled**) and `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68) + add(Bar, n=0.5 fixed, q<0.4, PA≈Stage-1 PA, Re_init=from the first round's bar-feature inner/outer radii)` (only when the original-image bar feature is strong). Measure Re_init on the first round's (single-Sersic) residual map, px. The `PA` here uses the **N=+Y contract** (see the 🔑 PA Convention); Stage-1 `detect_bar_lopsidedness`'s `bar.pa_deg` can be used directly.
- **Neither detected** → **1 candidate**: `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68) + add(Bulge, n=4 fixed, Re_init=from the first round's central-core radial measurement, px)`. (At depth=1 build the Bulge skeleton first; bar exploration waits for depth≥2 on residual evidence — it is not closed by a Stage-1 non-detection.)
- **Exception (elliptical + point core)**: if the single-Sérsic fit already looks elliptical-clean (outskirts reduced to noise, b/a > 0.5, no disk/bar signatures) and the only leftover is a compact central positive residual / 1D-profile spike within <5 px, give instead **1 candidate**: `add(agn, psf, x_px/y_px = core peak px, mag ≈ small-aperture estimate)` — NO convert primitive, NO re/n/q/pa_deg fields. {singlesersic, agn} is the legal elliptical-with-point-core terminal state (the expert-typical answer for such galaxies); do not force a disk skeleton on them.
...
Output **2–3** candidates. Typical directions: fix constraints, release/tighten a parameter, add a compact point-source component (an AGN, `psf` — under the Bulge-Re-collapse admission rule, or on a SingleSersic-only parent via the central-spike branch, which needs NO conversion), switch a component's model type.
...
- **Follow the component-addition order** (by companion location):
    ...
    - i.e. **embedded companions come after Bulge/Bar**; outer companions are unconstrained. Bar/Lens/AGN recognition must follow the `<Overall workflow of galaxy component analysis>` section.
    - **Lens candidate timing (two independent paths; either triggers)**:
        - **Path A (inferred from a Bar anomaly)**: when the parent Bar is physically anomalous (`Re_bar ≳ Re_disk(=1.68·Rs_disk)` or `q_bar ≳ 0.5`, i.e. the Bar is being forced to fit a Lens structure), generate a split candidate: `tune(Bar, split→Bar+Lens)` or `add(Lens, n<0.5 free, q>0.5, Re_init from bump peak radius/2, inside the bar–disk interval)`.
        - **Path B (1D-profile bump)**: when the 1D residual curve shows a broad positive bump about 1.5–2.5·Re_bar from the centre (see the Lens 1D Profile Bump Trigger Rule), generate an `add(Lens, n<0.5 free, q>0.5, Re_init≈bump peak radius/2)` candidate (px) even with perfectly normal Bar parameters.
        - The Lens uses `sersic`, n free (toggle=1) with prior n<0.5, Re satisfying the total order `Re_disk > Re_lens > Re_bar > Re_bulge` (compare only the existing central components; drop the missing ones and require decrease outward within the 3% tolerance), q>0.5, concentric with bulge/bar/disk.
...
- **Directional diversity** (with multiple candidates): candidates must span **clearly different** directions. Typical contrasting pairs:
    - "add a component" vs "tune a parameter" (e.g. +AGN(psf) vs release bulge_n)
    - "fix constraints" vs "change model type" (e.g. fix the bulge↔disk concentricity vs switch Disk→edgedisk)
    - "Occam" vs "deepen" (e.g. remove(agn) vs tighten the bulge_Re cap)
...
- **expected_C'**: the expected component inventory after applying the action (e.g. `{Disk, Bulge, Agn}`)
- **expected_behavior_tag**: expected fit-behaviour tag (short snake_case, e.g. `bulge_n_free_release`, `agn_add_psf`, `bar_pa_correct`, `edgedisk_switch`, `constrain_fix_concentric`, `occam_remove_agn`, `disk_add_fourier_f1`)
...
- **primitives**:
  1. <add|remove|tune>(<target>, <key params>)   ← add() on a target with Re must include the Re_init px value (AGN exempt; see the 🔑 Re Convention)
...
- **Physicality-verdict review (hard requirement, first item)**: the output starts with a properly formatted `## Physicality Verdict` block (all three fields verdict / failed_checks / swap_hint present), and the nested-containment check (check 1), the axis-ratio hard limits (check 2: bulge q ≥ 0.3, bar q ≤ 0.7), the globally-unblemished check (check 6) and the outermost-component out-of-bounds check (check 7) really were performed pair by pair against the ellipses, the legend q values and the panel boundary. On verdict=FAIL, confirm that at least one candidate this round repairs failed_checks (with swap_hint=disk_bulge_swap, include a disk ↔ bulge label-swap candidate); a verdict=PASS that contradicts Phase-1 notes of "Re total-order inversion", a bulge q < 0.3 / bar q > 0.7 legend value, or described ellipse crossing/poking/offset/out-of-panel features is self-contradictory — fix one of the two.
- **add() Re completeness (hard requirement, item by item)**: for every `add(<Type>)` primitive — whenever the target type **has a physical Re quantity** (Bulge / Bar / Lens / Companion / Disk and other sersic/expdisk components), the primitives **must** explicitly include the `Re_init` px value (with the measurement-source panel); no min/max bands are required or used (initial values only — the transcription layer ignores any declared band). An add() candidate missing Re is **invalid** and must be rewritten after returning to Phase 1 for the radial measurement (see the add()-completeness clause of the 🔑 Re Convention). **`add(AGN)` (psf component, no Re quantity) is exempt — you must not invent an Re for an AGN.** If a prior value from the Re total order was used because the feature could not be measured, this must already be declared in physical_motivation (see the degradation clause); otherwise it is equally a violation.
- **Re_init adjacency review (hard requirement, item by item)**: for every add() candidate carrying Re_init, insert the new component at its proper place in the total order `Re_disk > Re_lens > Re_bar > Re_bulge` and check the adjacent inner/outer components against the Model-panel legend's converged `Re(px)` — `adjacent inner Re < Re_init < adjacent outer Re` must hold (see the total-order adjacency clause of the 🔑 Re Convention). A candidate violating adjacency (e.g. bar Re_init < bulge Re, bar Re_init > disk Re) is equally **invalid** and must move Re_init into the interval per the adjustment rule.
...
- **Closed-alphabet review (hard requirement)**: every `add()` target this round belongs to the Candidate Space Alphabet's legal types (Disk/edgedisk/Bulge/Bar/Lens/OuterDisk/AGN/companion) — the AGN (`psf`) is the **sole** central point-source component and may be added only under the Bulge-Re-collapse admission rule or, on a SingleSersic-only parent, the central-spike branch. Any candidate adding a central point source / compact core under any other name or type must be rewritten as a compliant `add(AGN, psf)` candidate or deleted.
...
- **SingleSersic→Disk conversion review (hard requirement)**: every candidate that adds a **central** component to a SingleSersic parent bundles `tune(singlesersic→disk: expdisk, Rs = fitted Re / 1.68)` — `add(agn)` is the sole exception and must NOT bundle it ({singlesersic, agn} is legal as-is); no candidate's `expected_C'` is a multi-component model containing a sersic named `disk` or a `singlesersic` alongside anything but the AGN (invalid solution-space states the orchestrator rejects before fitting)
...
- **Lens Re inflation trigger review (hard requirement)**: if this round's supplement reports the lens Re reaching the default cap or lens_Re ≥ disk_Re (Re-ordering FAIL), confirm the output contains the three competing paths A (re-initialise lens Re ≈ 0.9×Re_above as a warm start) / B (grow disk Re; inapplicable when the disk itself is at the cap) / C (remove the lens) (see the Lens Re Inflation Trigger Rule). If one is missing, its waiver reason must be **explicitly stated** in a Candidate's physical_motivation; silent skipping is forbidden. (Candidate D / band relaxation is obsolete: no candidate-declared bands exist, every band is the default bound set.)
- **Bound-hit review**: bound hits (always default/original bands now) are **note-level facts** in the supplement — no relaxation candidates are required or meaningful (initial values guide the fit, never bands); treat pins as diagnostic information for your structural reasoning (e.g. Re pinned at the ≲1 px PSF-scale floor is a point-source identity question).
- **Mech-check repair menu review (hard requirement)**: the supplement's mechanical physicality table (code-computed) may flag the expert-round-2 families — **onion** (area ordering Re²·q of adjacent chain pairs, always on; directional 2·Re crossing — skipped for disk q < 0.3 / the edgedisk slot unless the flat disk contradicts the image's outer-isophote anchor, and exempts a round bulge laterally wider than a flat bar/lens), **disk_shape_inconsistency** (fitted disk q far flatter than the image anchor, Stage-1 not edge-on — note-level/advisory: q_iso is a coarse anchor; does not veto PASS and does not mandate a repair candidate), **shape priors** (bulge q < 0.3 hard / 0.3–0.4 note; lens q ≤ 0.5; bar q > 0.7; q_bar ≥ q_lens), **profile priors** (lens n ≥ 0.6; outerdisk n ≥ 1), **μ0 ordering** (μ0_agn < μ0_bulge < μ0_disk; μ0_bar ≤ μ0_disk/0.90), **flux shares** (bar 10–40%, lens 5–35%; f_lens < f_bar); bound pins are note-level facts only (no candidate-declared bands exist — every band is the default bound set). For every fired family confirm the output contains at least one mapped repair candidate: bulge q<0.3 → re-identify as bar (`tune(bulge, n=0.5 fixed)` / `remove(bulge)+add(bar)`) or add a rounder bulge; when the flat "bulge" is also compact (Re at the PSF-scale floor / <0.5 px) or a central <5 px spike persists, the competing collapsed-bulge replacement `remove(bulge)+add(agn, psf)` is likewise mapped (bare `remove(bulge)` then requires an explicit waiver — bulge-compensation physics outranks plain removal); bulge n at the 8 cap (free n pinned at the default upper bound) → by accompanying evidence: compact bulge + central spike/sharp core → `add(agn, psf)` + re-warm-start the bulge n (≈2–4, cusp-mimicry evidence); large-Re bulge + broad mid-radius 1D mismatch → the disk-role paths (`tune(disk, larger Re)` / mag reapportionment); genuinely concentrated giant elliptical with clean residuals → explicit waiver (BCG/cD n>8 is legal); raising the cap or freezing n=8 is NOT a repair; bar q>0.7 → round-bar degenerated → `remove(bar)+add(lens, n<0.5, q>0.5)` or `tune(bar, q≈0.35)`; lens q≤0.5 → `tune(lens, q≈0.7)` or bar re-identification; q_bar≥q_lens → competing q-separation or remove the weaker; onion area inversion / crossing → re-initialise the inner Re smaller (warm start ≈0.9× the outer's extent along the crossing direction) / grow the disk Re / re-identify; disk_shape_inconsistency → (advisory, not mandatory) re-round the disk toward q_iso with flux reapportionment, or re-identify the disk↔lens/outerdisk roles; lens n≥0.6 → `remove(lens)` or re-identify; outerdisk n≥1 → `tune(outerdisk, n≈0.5)` or remove; μ0 inversion → `tune(<component>, mag)` reapportionment (disk_bulge_swap when the Re order is also inverted; `remove(AGN)` for a weak AGN); flux-share fires → remove the weaker or re-identify (lens≥bar), disk-Re bottleneck `tune(disk, larger Re)` (bar>40%), or `remove(bar)` only when bar <10% AND its radial zone shows no residual signature. Absence without an explicit waiver in physical_motivation is a violation.
...
- **Flux-misallocation review (hard requirement)**: if this round's supplement contains "Flux-misallocation signals" (trigger A, B or D), confirm the output contains a `tune(component, mag_init=…)` candidate **or** an explicit waiver in some candidate's physical_motivation; silent skipping is a violation. The mag edit must be initial-value only — no `.cons` mag bands.
