...
2. **Axis-ratio hard limits (hard criterion; a hit is FAIL evidence)**:
   - **Bulge q ≥ 0.5**: a fitted bulge with b/a < 0.5 is physically inconsistent with a bulge — a genuinely flattened central component is bar/lens territory, not a bulge. A hit enters failed_checks as FAIL evidence (repair direction: the Flat-Bulge → Bar Candidate Trigger Rule).
   - **Bar q ≤ 0.6**: a fitted bar with b/a > 0.6 has lost its elongated bar identity (round-bar degeneration). A hit enters failed_checks as FAIL evidence (repair directions: tighten the bar q upper bound, or the Bar+Lens split paths of the Lens trigger rules).
...
- **Scope and AGN exemption**: this convention (triplet requirement included) applies to all components **with a physical Re quantity** — Bulge / Bar / Lens / Companion / Disk and other sersic/expdisk components. **The AGN uses the `psf` component type and has no Re parameter**: `add(AGN, ...)` candidates are exempt from the Re-triplet requirement, and you **must not invent an Re for an AGN**.
...
- **Mech-check repair menu review (hard requirement)**: the supplement's mechanical physicality table (code-computed) may flag the expert-round-2 families — **onion** 2·Re ellipse crossing (the code skips this family for disk q < 0.3 / the edgedisk slot, and exempts a round bulge laterally wider than a flat bar/lens), **shape priors** (bulge q < 0.4; lens q ≤ 0.5; bar q > 0.6; q_bar ≥ q_lens), **profile priors** (lens n ≥ 0.6; outerdisk n ≥ 1; lens flat degeneration n ≤ 0.1 ∧ Re ≥ 0.9·cap), **μ0 ordering** (μ0_agn < μ0_bulge < μ0_disk; μ0_bar ≤ μ0_disk/0.90), **flux shares** (bar 10–40%, lens 5–35%; f_lens < f_bar). For every fired family confirm the output contains at least one mapped repair candidate: bulge q<0.4 → re-identify as bar (`tune(bulge, n=0.5 fixed)` / `remove(bulge)+add(bar)`) or add a rounder bulge; bar q>0.6 → round-bar degenerated → `remove(bar)+add(lens, n<0.5, q>0.5)` or `tune(bar, q, [0.2,0.5])`; lens q≤0.5 → `tune(lens, q, [0.55,0.9])` or bar re-identification; q_bar≥q_lens → competing q-separation or remove the weaker; onion crossing → tighten the inner `re_max` (≈0.9× the outer's extent along the crossing direction) / grow the disk Re / re-identify; lens n≥0.6 → `remove(lens)` or re-identify; flat degeneration → the Lens-Re-inflation paths A/B/C/D; outerdisk n≥1 → `tune(outerdisk, n, [0.1,0.8])` or remove; μ0 inversion → `tune(<component>, mag)` reapportionment (disk_bulge_swap when the Re order is also inverted; `remove(AGN)` for a weak AGN); flux-share fires → remove the weaker or re-identify (lens≥bar), disk-Re bottleneck `tune(disk, larger Re)` (bar>40%), or `remove(bar)` only when bar <10% AND its radial zone shows no residual signature. Absence without an explicit waiver in physical_motivation is a violation.
