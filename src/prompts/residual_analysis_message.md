...
2. 其次，细致分析星系所包含的成分类型。采用逐成分递增的方式进行，保证拟合的稳定性；成分添加逻辑
    ...
    3. 观察原图、model 图、1D 与 2D 残差图，确定是否存在预期的成分类型（也是成分添加的顺序）？
        ...
        3. 伴星系认定并添加
            1. 如果明显中心星系周围存在点源或者展源，且未被Mask，会影响拟合质量，需要添加Seric或者PSF吸收他们的影响
        ...
        5. Nucleus 成分的认定方法 (这个成分的添加要放在最后， 并且需要有成分的证据佐证Bulge不足以满足拟合要求。)
    ...
    5. 调参策略：
        ...
        2. 对于未mask的伴星系，影响拟合质量，需要优先处理
            - 添加sersic成分拟合伴星系（注意坐标偏移）
        3. 如果存在sky参数偏离背景噪声较大，优先处理
            - sky 参数fix到背景均值
        4. 拟合异常的调参策略：
            1. 当遇到 Sersic 的 Re 很小时（比如小于 0.2px）或者 n >> 20 时， 不要机遇删除改成分或者替换为 PSF；先要经过细致的调参确认，以排除拟合的问题。
                - 策略1: 1D SB profile 图上，查看 SKY的星等是否与背景噪声相当（背景噪声一般为 Data曲线的最右侧几个数据的平均值）
                    - 如果 SKY远低于背景，需要将 SKY 背景参数调高，具体的参数值，可以参考 1D SB profile图上 SKY与 Data的距离， 根据星等倍数对应调整。调整后固定参数，避免拟合过程跑偏。
                    - 如果单次设置数值不正确，需要多次调整，达到背景均值为止。
        ...
        5. 如果新增成分拟合的位置与原来的偏差较大（比如偏离距离大于 Re）；可以考虑添加一个约束，限制偏移量
        6. 当遇到某个通量占比过低，同时残差特征又预算该成分的存在，这预示着 各成分的 mag 初值需要重新分配。
            - 在使用奥卡姆剃刀之前，需要经过充分调参验证。
        7. 对于一般情况，可以直接让 Re、Mag 和 n 都自由拟合; 对于可能包含多个成分的复杂情况（特别是残差非常大的情况），Disk/Bulge可以才考虑固定 n 的, Re 和 Mag 参数拟合充分后，再放开n。都拟合到位了再考增加下一个成分。
