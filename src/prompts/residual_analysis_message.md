...
2. 其次，细致分析星系所包含的成分类型。采用逐成分递增的方式进行，保证拟合的稳定性；成分添加逻辑
    ...
    3. 观察原图、model 图、1D 与 2D 残差图，确定是否存在预期的成分类型（也是成分添加的顺序）？
        ...
        3. 伴星系认定并添加
            - 认定方式：原图上：中心信息周边存在独立的明显亮斑与区域,并且残差图的相同位置存在明显的正残差。
            - 如果中心星系周边邻近区域内存在点源或者展源，且未被Mask，可能会影响拟合质量，需要添加Seric或者PSF吸收他们的影响
            - 至于是否接受该伴星，遵循遵循奥卡姆剃刀原则
        ...
        5. Nucleus 成分的认定方法 (这个成分的添加要放在最后， 并且需要有充分的证据佐证Bulge不足以满足拟合要求。比如1D profile上 0位置还有明显尖峰的，不能只通过残差图猜测)
    ...
    5. 调参策略：
        ...
        3. 拟合异常的调参策略：
            1. 当遇到 Sersic 的 Re 很小时（比如小于 0.2px）或者 n >> 20 时， 不要机遇删除改成分或者替换为 PSF；先要经过细致的调参确认，以排除拟合的问题。
                - 策略1: 如果存在 SKY 成分， 参考 1D SB profile 图，查看sky background值，将SKY成分参数<sky>fix到 sky background值；如果未添加 SKY成分，该策略忽略。
                - 策略2：根据残差特点，调整 Disk 与 Bulge 的初始，这是必要步骤
                    - 比如：Bulge < 0.2 px， 但星系边缘还存在明显的正残差，说明 Disk的Re设置过低，同时挤压了 Bulge 的 Re，导致 Bulge 的 Re 过小。需要同步调高两者的 Re 初值
                    - 其他残差特征意味着需要其他调整, 比如：
                        - 如果外围存在明显的正残差环，说明 Disk 拟合不足，需要增大 Re来拟合正残差
                        - BAR的 PA数值明显偏离图像所示方向， 需要纠正 PA,设置合理值
                        - 各成分的 Re偏离目标区域明显,
                - 策略3：先将 Bulge 的n 固定为 4，等参数正常后， 再释放 n.
                    - 固定 n时， Re 设为 Free, Disk 等成分参数也 Free, 重新拟合，期望能够重新分配参数，避免参数异常。
                    - 参数正常后，释放 Bulge 的 n；再重新拟合
                - 策略4：星系拟合主体区域周边是否存在伴星，需要加入异常成分拟合伴星，排除干扰。（伴星再残差上体现为局部孤立点源，原图上对应一个亮斑）
                - 如果尝试完策略 1-4，所有方案都无法解决 Bulge 参数异常 (Re < 0.1 或者 n >> 20 )
                    - Re < 0.1  根据物理意义分析，才考虑把 Bulge 换成 PSF 进行拟合; 不要一上来就添加 PSF, 仅当排除其他所有可能性，才建议添加 PSF
                    - 以上所有策略都无法解决，参数简并；或者依旧没有物理意义。 才考虑剔除该成分。
        4. 如果新增成分拟合的位置与原来的偏差较大（比如偏离距离大于 Re）；可以考虑添加一个约束，限制偏移量
        5. 当遇到某个通量占比过低，同时残差特征又预算该成分的存在，这预示着 各成分的 mag 初值需要重新分配。
            - 在使用奥卡姆剃刀之前，需要经过充分调参验证。
        6. 对于一般情况，可以直接让 Re、Mag 和 n 都自由拟合; 对于可能包含多个成分的复杂情况（特别是残差非常大的情况），Disk/Bulge可以才考虑固定 n 的, Re 和 Mag 参数拟合充分后，再放开n。都拟合到位了再考增加下一个成分。
