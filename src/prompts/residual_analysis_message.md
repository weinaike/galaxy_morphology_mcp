...
2. 其次，细致分析星系所包含的成分类型。采用逐成分递增的方式进行，保证拟合的稳定性；成分添加逻辑
    ...
    3. 观察原图、model 图、1D SB Profile 、 2D 残差图，确定是否存在预期的成分类型（也是成分添加的顺序）？
        1. Disk 成分与Bulge的认定方法
            ...
            - 双成分 Disk 采用 expdisk， Bulge 先固定 n=4 或 1 进行拟合，初步分配 flux，正常后再 free n 进行重新拟合调整。（这个步骤是必要的）
        ...
        3. 伴星系认定并添加条件
            - 认定条件：原图上中心星系周边存在独立的明显亮斑与区域（要求足够亮，能够与中心星系亮度可比拟）,并且残差图的相同位置存在明显的正残差（会影响中心星系的拟合质量）。
            - 如果中心星系周边邻近区域内存在点源或者展源，且未被Mask，且明显影响拟合质量时，才考虑添加Seric或者PSF吸收他们的影响
            - 对于暗的、不明显的伴星系，或者距离较远的伴星系，即使残差图上存在一些正残差，对于中心星系影响不大，选择不添加
            - 至于是否接受该伴星，严格遵循奥卡姆剃刀原则
    ...
    4. 调参策略：
        ...
        3. 当遇到 Bulge 的 Re 很小时（比如小于 0.2px）或者 n >> 20 时，先要经过多轮的调参策略尝试，以避免落入局部收敛陷阱。
            - 尝试 1：Bulge 先fix n = 4或者 1 进行拟合，分配 flux；正常后，再基于拟合后结果参数的，free n，重新尝试。
        ...
        4. 如果同星系新增成分，要求成分参数<x><y>与原有成分的<x><y>绑定，即保证她们同心（要求<x>和<y>同时绑定，严禁仅绑定一个变量）
        5. 当遇到某个通量占比过低，同时残差特征又预示着该成分的存在，各成分的 mag 初值需要重新分配。
        6. 对于一般情况，可以直接让 Re、Mag 和 n 都自由拟合; 当前成分拟合收敛合理后，再考增加下一个成分。
