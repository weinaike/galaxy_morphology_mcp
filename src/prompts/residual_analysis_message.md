...
2. 其次，细致分析星系所包含的成分类型。采用逐成分递增的方式进行，保证拟合的稳定性；成分添加逻辑
    ...
    3. 观察原图、model 图、1D SB Profile 、 2D 残差图，确定是否存在预期的成分类型（也是成分添加的顺序）？
        1. Disk 成分与Bulge的认定方法
            ...
            - 另外从原图和 model 中的轮廓也是一种很好的辅助判断，是否需要增加成分，来平衡外部大的轮廓和内部的亮度区域。才能是的两者亮度轮廓达到基本相似
        2. Bar 成分认定方法与添加方法
        ...
        3. 伴星系认定并添加条件
            - 认定条件：原图上中心星系周边存在独立的明显亮斑与区域,并且残差图的相同位置存在明显的正残差。
        ...
        5. Nucleus 成分的认定方法（必须满足认定条件）
            - 认定条件：1D profile残差图（DATA-MODEL）的左侧位置（0附近）要求出现明显尖峰的。
    ...
    4. 调参策略：
        ...
        3. 当遇到 Sersic 的 Re 很小时（比如小于 0.2px）或者 n >> 20 时，先要经过细致的调参确认，以排除拟合的问题。
            - 策略：根据残差特点，调整 Sersic 的初始，这是必要步骤
                - 如果：Bulge < 0.2 px， 但星系边缘还存在明显的正残差，说明 Disk的Re设置过低，同时挤压了 Bulge 的 Re，导致 Bulge 的 Re 过小。需要同步调高两者的 Re 初值
                - 如果：外围存在明显的正残差环，说明 Disk 拟合不足，需要增大 Re来拟合正残差
                - 如果：BAR的 PA数值明显偏离图像所示方向， 需要纠正 PA,设置合理值
                - 如果：Sersic 的 n与目标值偏离很大， 考虑先fix n, 拟合一轮；在拟合结果的基础上，free n，重新尝试。
