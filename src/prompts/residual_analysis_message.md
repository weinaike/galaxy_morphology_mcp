...
- **多波段下偏心（lopsidedness）的处理时序：** 多波段 workflow 中，偏心的**检测**在阶段一由 `detect_galfits_bar_lopsidedness` 完成（跨波段 OR-logic），结果固化在 `working_note.md`；偏心的**拟合**（Disk 改 `sersic_f` + m=1）在阶段三锁定最佳结果后由 `fourier_mode_analysis` 评估决定。**迭代分解阶段（阶段二）不要主动建议添加 Fourier 成分**，仅引用 `working_note.md` 中已有的偏心检测结果即可。
...
2. 其次，细致分析星系所包含的成分类型。采用逐成分递增的方式进行，保证拟合的稳定性；成分添加逻辑
    ...
    3. 观察原图、model 图、1D SB Profile 、 2D 残差图，确定是否存在预期的成分类型（也是成分添加的顺序）？
        ...
        3. 多成分分解流程
            - Disk 成分与Bulge的认定方法
                ...
                - 残差图中,中心星系边缘位置出现明显的环状正残差
                    - 可以考虑增大Disk 的Re/Rs, 让Disk承担更多的外围拟合任务, 同时调整其他成分参数重新分配能量。
                    - 新增Outer Disk成分，采用Sersic n<1的成分，来专门拟合外围的环状残差。
                - Disk + Bugle 双成分拟合时，Bulge的 q < 0.5, 即使残差中为观察到明显的 bar成分，也可以将先将 Bulge 切换为 Bar（即固定n=0.5） 进行拟合; 在添加额外的 Bugle，构建为 Disk + Bar + Bulge 三成分。
            - Bar 成分认定方法与添加方法
            ...
            - 伴星系认定与添加条件
                - 有影响伴星系认定条件：
                    ...
                    - 能量限制：伴星系的星等比中心星系的星等差异不能过大（通常不超过 3 个星等），否则对中心星系的拟合影响较小，可以选择不添加。
                - 伴星系添加的方法
                    ...
                    - lyric文件中添加伴星系，必须对有效半径的上限值作严格的限制（通常不超过3像素），防止主星系残差的干扰，将伴星系拉得过大。
            - Nucleus 成分的认定方法（必须满足认定条件）
                - 认定条件1：1D profile残差图（DATA-MODEL）的左侧位置（<5pix内）要求出现明显尖峰的，且无坍缩的Bulge成分。
                - 认定条件2: 如果有坍缩的 Bulge 成分（Re < 0.2px；多波段拟合下要求每个波段 WCS 转换后全部 < 0.2 px），也可以考虑是 Nucleus 的候选（但要求满足调参策略第 3 点的要求）。
            ...
            - 高阶残差：偏心成分、壳层（Shells）或潮汐尾 （根据科学目标而定，只有用户特别要求，才需要考虑高阶残差）
                1. 偏心（Lopsidedness）特征：明显的“偶极” (Dipole) 模式，即中心星系的残差不对称，原图上星系的“一边比另一边更重、更亮或延伸得更远”，呈现出“水滴状”或“鸡蛋形”的拉伸。
    ...
    4. 调参策略：
        ...
        3. 当遇到 Bulge 的 Re 很小时（比如小于 0.2px；多波段拟合下指每个波段 WCS 转换后均 < 0.2 px）或者 n >> 20 时，先要经过多轮的调参策略尝试，以避免落入局部收敛陷阱。
            ...
            - 如果尝试都无法改变 Re 过小的事实，可以考虑将 Bulge 成分换成 PSF 模型进行拟合。（如果 PSF 为了代偿 Bulge缺失, PSF的物理意义优先，不可因而 BIC或者 mag弱而删除）
...
+ 如果Bulge拟合出来的结果, re很小（如 < 0.2 px；多波段拟合下要求每个波段 WCS 转换后全部 < 0.2 px），意味着这个成分拟合的是一个点源，可以换成PSF model去拟合。（这个方法仅当所有策略都已经使用，都无法改善拟合效果时才能使用）
+ 对于盘星系而言的物理意义描述而言，结构优先级 （Disk + Bar + Bugle）>（Disk + Bar + PSF）>（Disk + Bar）=（Disk + Bugle）>（Disk + PSF）> Disk; 所以这些具有物理意义的成分添加上后，残差质量未变差就应该保留。
...
+ 盘星系的 Bulge, Bar，Nucleus 成分在 1D SB Profile 中的能量成分占比大于 0.01(1%)， 其物理意义上就需要保留。

# 奥卡姆剃刀原则

## 奥卡姆剃刀的使用场景
必须严格遵守奥卡姆剃刀原则的使用场景条件：
- 奥卡姆剃刀，只适用于 Nucleus/AGN 成分或者伴星系成分的增减，
