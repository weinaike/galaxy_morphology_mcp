...
- 基于阶段三的分析，推理下一步动作（最紧迫）：
    ...
    - 必须确认以下内容：
        ...
        - 2. 如果添加伴星系要求严格符合“有影响伴星系认定条件”与“伴星系添加的方法”，特别需要确认距离是否满足要求,
            - 伴星系/fourier mode 添加不受中心星系增减成分（小于或等于 1 个成分）的约束。
        - 3. 同心成分之间的<x><y>应该绑定在一起，x与y要同时绑定，不能只绑定单坐标。
        - 4. 对于单波段GALFIT拟合，Disk 应该是用expdisk模型或者edgedisk模型，不应该使用sersic；对于多波段GALFITS拟合，确认的Disk成分固定sersic n=1.
        - 5. 如果要将 Bulge 替换 PSF, 在这之前需要先确认，是否已经经过多轮调参尝试，试图将Bulge的 Re 回归到合理范围。（具体调参方法见：<调参策略>）
        - 6. 严禁滥用奥卡姆剃刀原则
            - 如果 Bulge 最终坍塌采用 PSF 替代的，不能再使用奥卡姆剃刀指标评估其是否保留。（其作为 Bulge的替代的物理意义优先）
            - Bar, bulge, Disk, Fourier mode 这些成分的增减，只取决于是否包含物理含义和残差特性。 不需要参考BIC指标。
