...
**阶段二：参数与运行状态审查**
1. 回顾历史轮次的目标与进展，特别是working_note记录中的round 0: 的预期成分的论证进展。
2. 描述当前拟合的状态（如：当前拟合的物理成分有哪些？参数的收敛情况如何？是否存在明显的异常参数值？等）。
3. 遇到异常情况，需要先分析原因执行调参方案，不要急于使用奥卡姆剃刀。
...
- 必须确认以下内容：
    ...
    - 确认 Working Note 的 Round 0 关于detect_bar_lopsidedness工具的检测结果。
        - 如果Round 0描述 lopsidedness 被检出。 m=1 Fourier Mode 成分应该被最高优先级添加，只有证明m=1 Fourier mode 成分会导致拟合不合理，才需要去除。
        - 如果Round 0描述 Bar 被检出，在多成分构建时，要重点参考参考该结果。
...
- 基于阶段三的分析，推理下一步动作（最紧迫）：
    ...
    - 必须确认以下内容：
        ...
        - 6. 使用奥卡姆剃刀原则时，需要校验当前场景是否符合奥卡姆剃刀的使用场景条件要求
        - 7. 必须提供中心星系所以包含的完整成分类型预测。
        - 8. 对于判定可能存在的 lopsidedness的星系，在添加其他成分之前，需要先添加 m=1 Fourier mode 吸收偏心影响
        - 9. m=1 Fourier mode 的 amplitude 大于 阈值 0.02 就可以保留。不需要移除
