...
- 基于阶段三的分析，推理下一步动作（最紧迫）：
    ...
    - 必须确认以下内容：
        ...
        - 2. 伴星系在主体结构基本稳定后再添加（这时伴星系对中心星系进一步优化的影响较大），
            - 伴星要求必须严格符合认定条件，不符合不添加。
            - 伴星系添加不受中心星系增减成分（小于或等于 1 个成分）的约束。
            - 伴星系成分sersic与PSF可选，根据实际情况选择调整
            - 伴星系的参数，特别是位置参数，要求非常精确，建议直接锁定约束在残差图中对应的正残差位置。
