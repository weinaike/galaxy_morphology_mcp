...
Focus on fitting the physical components of galaxies — disk, bulge, bar, AGN, lopsidedness (F1) — adding model components driven by residual features; higher-order residual features may be left unfitted. **Branch restriction (`restrict-disk-bulge-bar-agn`)**: the solution space on this branch allows only Disk (expdisk) / Bulge / Bar / AGN (psf) / F1 / SingleSersic (+ the fixed sky); **Companion, Lens, OuterDisk and edgedisk are outside the solution space** — companion/lens/envelope-type residual features are described as context but left unfitted, and no candidate targeting them may be generated, transcribed or executed (see CLAUDE.md's solution-space definition).
...
- **Refutation validity [sub-converged] (hard; guards the ledgers against invalid evidence)**: a fitted round is **valid evidence** only when `run_galfit`'s `fit_statistics.convergence.flag` is `ok` (the tool compares converged galfit.NN values against the input feedme and flags rounds whose ≥2 free **non-position** parameters — mag/re/n/q/pa — ended exactly at their input values as `sub-converged`; centre x/y freezing is normal warm-start behaviour and excluded). A **[sub-converged]** round: (i) is recorded in the result ledger tagged `[sub-converged]` with the frozen-parameter list; (ii) must **NOT** be appended to `[Refuted hypotheses]` (neither its ΔBIC nor its physicality FAIL may ground a refutation entry — a hypothesis "tested" by an optimiser that never explored is untested); (iii) does **NOT** count toward the per-combination attempt cap; (iv) does not update s\* on its numbers alone. When a sub-converged round's direction is re-proposed, the orchestrator re-runs it once from a corrected configuration (proper `to`-form bands / adjusted initial values), and the re-run counts as a fresh attempt. Real incidents motivating this gate: KILOGAS_432 A.10 and KILOGAS_353 A.6/A.11 — invalid "bulge-n release refuted" entries from frozen optimiser runs, each later overturned at ΔBIC_eff −1063 / −465.
...
b. **Execute the transition T(s, a)**:
    ...
    2) **Concentric-constraint check for main-galaxy components (hard; execute regardless of whether a mentions constraints)**: count the main-galaxy central components in this round's `_iter{global_iter_id}.feedme` (Disk/Bulge/Bar, i.e. `# STRUCTURE:` names disk/bulge/bar) as K:
       - **K ≥ 2**: **must** write `iter{global_iter_id}.cons` in the galaxy home directory, its syntax and full example strictly per the CLAUDE.md section "Concentric constraint for main-galaxy central components — `.cons` authoring specification" — chain the anchor (the Disk; absent one, the brightest central component) with all subordinate central components' numbers using `_`, and write the **paired** lines (both indispensable): `<chain>  x  offset` and `<chain>  y  offset`. Key points: the anchor's `1)` toggle stays `1 1` (once effective, the group translates jointly and the centre is optimised by the fitter); subordinates' `1)` rows stay `1 1` (GALFIT rewrites them to `2 2` as the constrained marker); the feedme `G)` item points at the file; any other `.cons` bounds this round (re/n/mag ranges) are **merged into the same file** (GALFIT loads exactly one constraint file). **Syntax warning**: the space-pair (`1 2 x 0.0 0.0`) and dash-pair (`1-2 x 0.0 0.0`) forms are **silently ineffective** on this machine's GALFIT (no error, but the centres drift) — strictly forbidden; the only effective form is the chain `offset` hard constraint.
       - **K ≤ 1** (a single component or the starting single sersic): no offset chain; a `.cons` may still be written for other bounds.
       - **Default bound set (hard; every round, independent of K)**: feedme parameter rows carry no bounds — without a `.cons` the solution space is open and the bound-hit diagnostics are blind. Per the CLAUDE.md "Solution space definition — mandatory default bound set", write the default `.cons` bounds for **every non-sky component**: `re` row (`max(0.1, 0.5×FWHM_PSF)` **to** half the fit-region side; FWHM_PSF from `fit_statistics.psf_fwhm`, fallback 1 px; expdisk rows in Rs = Re/1.68), `n` row (0.1 **to** 8, sersic components only), `q` row (0.05 **to** 1.0, shaped components only); centres per the offset-chain rule above, or a relative ±2 px window (`<N> x -2 2`, `<N> y -2 2`) when K ≤ 1. **⚠️ Encoding rule (empirical, this machine's GALFIT 3.0.5 — see CLAUDE.md's `.cons` numeric-band semantics note)**: absolute bands for `re`/`n`/`q` MUST be written in the **`to` keyword form** (`<N> re <min> to <max>`) — a row with two bare numbers is parsed as **offsets relative to the component's input value** ([input−a, input+b]) and never enforces the intended absolute band; position windows are intentionally relative (bare ±offsets around the input value). **Merged into the same `iter{global_iter_id}.cons`** (GALFIT loads exactly one constraint file). Provenance convention: these defaults count as "original" bounds in the d.ii bound-hit reporting; candidate-declared tightenings count as "self-imposed"; candidates may tighten but not widen past the default cap without an explicit declaration.
...
c. **Build the new state s'**: read reduced_χ² and BIC from the `run_galfit` return (`fit_statistics`'s `chisq1d_nu` and **`bic_eff`**, falling back to `chi2_nu` / `bic1d` respectively when missing; see the BIC Convention) **and the convergence flag `fit_statistics.convergence.flag`** — a `sub-converged` flag invokes the Refutation validity rule (formal definitions): the round enters the result ledger tagged `[sub-converged]` with its frozen-parameter list, grounds no `[Refuted hypotheses]` entry, does not count toward the combo cap, and must not update s\* on its numbers alone; `R'` is the archived comparison PNG; `C'`, `P'` come from the new feedme and the archived `output_param_file` (galfit.NN converged values). Round naming: within the branch, `branch.local_round` (A.2, A.3, B.1…; A.1 is taken by the first fit), decoupled from global_iter_id. s' has depth `depth + 1`.
d. **Candidate generation + physicality verdict of the fit (unconditional hard requirement — see §Diagnostic-First Principle of Candidate Generation; the two orthogonal sources merge into Step f for scoring and enqueuing)**: this step **must** run whenever Step b's fit succeeded (the b.5 failure branch excepted), regardless of the verdict, whether the BIC got worse, whether parameters hit bounds, whether unspent candidates remain in the queue, or how tight the budget is. Each main-loop round's candidates come from two orthogonal sources — d.i is VLM-driven from residual-image visual analysis plus the **physicality verdict**; d.ii is orchestrator-driven from objective thresholds on the fit numbers. The two merge and pass through exactly the same dedup / scoring / truncation rules (Step f), competing equally for the queue.
    - **d.i VLM visual candidates + physicality verdict**: with the new comparison image as `comparison_file`, the new feedme as `feedme_file`, the archived galfit.NN as `fitted_param_file`, and the archived summary as `summary_file`, call:
        ...
        - **Verdict block missing or unparseable** (VLM ignored the format): the orchestrator applies a minimal fallback — call `check_feedme_file` for the canonical inventory and compare the main-galaxy total order `re_disk > re_bar > re_bulge` numerically (expdisk via `re_effective_px`; no other tools; this is only the minimal numerical subset of the VLM's nesting check and cannot catch morphological violations like poking-out or crossing); record the result in working_note tagged "[verdict fallback]". Normally rely on the VLM; this fallback only prevents a format miss from disabling the gate.
    ...
    - **d.ii Orchestrator numeric-rule candidates**: from s''s fit numbers (galfit.NN + feedme toggles + `.cons` bounds) the orchestrator runs objective numeric checks and reports the **phenomena and numerical facts** to the VLM; **what candidates get generated is decided entirely by the VLM per `beam_action_generation_prompt_galfit.md`** (the orchestrator neither generates nor hints directions). These checks target "visually present but numerically suspicious components" — the VLM, seeing a component in the image, tends to keep it, but if the numbers are suspicious (e.g. extremely low flux, or a base component's Re too small and compensated by an extended one), the orchestrator hands the objective numbers to the VLM, which combines them with the original image to decide whether to generate adjustment candidates (removal or parameter tuning). Currently defined triggers:
        - **Disk-Re bottleneck check (bound-hit extended component + Re/flux degeneracy)**: if s' has a bar, read its fitted Re and its re upper bound in `.cons` (without a bound, the "bound-hit" sub-criterion fails), plus the disk's Re and Mag. When the bar Re hits the cap (fitted value ≥ 0.98 × cap) **and** either sub-criterion below holds, call it a "disk-Re bottleneck hit" — the extended component wants to be bigger but is held back by the disk Re, an objective signal that the disk Re is too small:
          - **Sub-criterion A (Re degeneracy)**: `Re_bar / Re_disk ≥ 0.85` (the two are highly degenerate along Re; the bar nearly catches the disk).
          - **Sub-criterion B (flux approaching or exceeding)**: `Mag_bar ≤ Mag_disk + 0.2` (the bar flux ≥ ~83% of the disk, or more). This catches the degeneration where the bar is forced to take over the disk's outer flux.
          - **Fact report** (after either sub-criterion hits; if the disk Re is also bound-hit, do not trigger — the disk has no room): write the bottleneck **facts** into `local_state_description` in the form: "Disk-Re bottleneck signal (objective facts): bar_Re=5.0px at the cap (re_max=5.0px), Re_bar/Re_disk=0.93 [≥0.85 (Re degeneracy hit) / <0.85], bar_Mag=16.86 [≤ disk_Mag+0.2=17.28 (flux-approaching hit) / above]. disk_Re=6.1px (not bound-hit / unconstrained)." — **no operational suggestion**; generating `tune(disk, larger Re)`-type candidates (1D-curve visual confirmation included) is decided by the VLM per the Disk Outer-Flux-Deficit Trigger Rule. Such VLM candidates enter the queue with a floor of **g ≥ 0.5** (mandatory retention — the floor affects ranking only, not generation rights).
          - **Rationale**: the rule targets the "bar inflating to compensate the disk's outer flux" degeneration — when the bar Re hits the cap and degenerates with the disk in Re or flux, the root cause is usually a too-small disk Re, not a genuinely oversized bar. The signal is fully objective (from the fit numbers) and does not rely on the VLM's visual reading of low-SNR outskirts (which has proven unstable — once the bar compensates the outer flux the 1D curve flattens and the visual check stops firing).
          - If s' has no bar, skip.
        - **Flux-misallocation check (numeric facts only; candidates decided entirely by the VLM per the beam prompt's Flux-Misallocation Trigger Rule)**: mag is free and warm-started every round, so the optimiser normally reallocates flux itself — this check surfaces the three basin-trap signals where it cannot, each reported as a bare fact with **no operational suggestion attached**:
          - **Signal A (faintness vs the disk)**: any main-galaxy component with `Mag_i − Mag_disk > 3` (flux < ~6% of the disk) — report ΔMag and the flux ratio `f_i/f_disk = 10^(−0.4·ΔMag)`. Whether that component's radial zone still carries significant residual features (≳ ±0.1 mag) is the VLM's Phase-1 judgement; the rule fires on faintness ∧ residual evidence together.
          - **Signal B (monotonic Mag drift)**: any component whose fitted Mag has moved monotonically in one direction by ≥ 0.3 mag per round over ≥ 2 consecutive rounds (read from the result ledger's state signatures) — report the trajectory (e.g. `bar: 17.52 → 17.96 → 18.14, +0.4/+0.2 mag/round`). Chronic flux bleeding is the bad-allocation signature, complementary to the frozen-parameter [sub-converged] flag.
          - **Signal D (sub-convergence re-run prescription)**: when this round is flagged `[sub-converged]`, the corrected-configuration re-run options (per the Refutation validity rule) include **mag initial-value reapportionment** alongside band re-encoding and init adjustment — report it as one of the standard prescriptions.
          - Format: "Flux-misallocation signals (objective facts): signal A — bulge ΔMag vs disk = 3.4, flux ratio 4.3%; signal B — bar Mag trajectory 17.52 → 17.96 → 18.14 (monotonic, ≥0.3 mag/round × 2); signal D — round flagged [sub-converged], mag reapportionment available in the re-run." On any signal, `tune(component, mag_init=…)` candidates are generated (or explicitly waived in physical_motivation) by the VLM per the beam prompt; **no mandatory-retention floor applies**.
        - **Generic bound-hit parameter scan (generalised rule; objective fact report, no candidates)**: each round the orchestrator compares s''s galfit.NN fitted values against the `.cons` **effective** bands (including `iter{n}.cons` and the input constraints) **parameter by parameter** (all components, all bounded parameters) — with the `to` form the written numbers are the effective band; a bare two-number row must first be decoded to `[input−a, input+b]` (see CLAUDE.md's `.cons` numeric-band semantics note; comparing against raw bare numbers produces phantom "bound hits") — listing every bound-hit parameter (|fitted − bound| ≤ 2%×|bound| or equal), and for each reports **five facts** objectively: parameter name and fitted value (px, per the pixel contract), the hit bound, the direction (upper/lower), the **bound provenance** (self-imposed = the current bound differs from the input configuration's original, produced by an earlier tightening/repair; original = same as the input), and the number of consecutive bound-hit rounds (from the working_note state ledger). Write the list verbatim into `local_state_description` (format: "Bound-hit parameter list (objective facts): bar_Re=9.0px at the cap re_max=9.0px [self-imposed (original 12.6px)] 2 consecutive rounds; bulge_n=8.0 at the cap n_max=8.0 [original] 4 consecutive rounds; …"), **with no candidate-direction suggestion or imperative attached** ("please generate / suggest generating / confirm generating / should relax" are all violations) — the generation duty is decided by the VLM per the tiers of the 🔑 Bound-Relaxation Rule (Tier 1 self-imposed mandatory / Tier 2 original competing / exemptions). **Unbounded parameters cannot hit a bound and never enter the list.**
        - Further objective numeric triggers (anomalous sky, etc.) may be appended here in the same pattern — **describe the phenomenon and numbers only; generation decisions always belong to the VLM**.
    - **Traceability**: the d.ii numeric checks (whether or not the VLM generated the corresponding candidates) are annotated "[orchestrator numeric-rule delegation]" in the relevant branch section of `working_note.md`, recording the measured values (disk-Re bottleneck: bar Re and re_max, disk Re and re_max, Re ratio, flux ratio, sub-criterion A/B hit; flux-misallocation: ΔMag/flux ratio per signal-A component, per-component Mag trajectory for signal B, signal-D prescription availability) and the VLM's visual-verification conclusions. This audits later decisions to keep/remove/adjust components.
...
4. **Bulge-n release compliance review (hard)**: if s\* contains a bulge with n still fixed at 4 (the depth ≤ 2 skeleton scaffold), the input ledger must contain at least one **valid** n-release attempt on that inventory — a round with bulge n free, `convergence.flag = ok` (not [sub-converged]), and its n band written in the enforced `to` form (`2 n 0.5 to 8`). A missing, mis-encoded or [sub-converged] attempt (see the Refutation validity rule) is a process violation: restart one beam round executing `tune(bulge, n free)` as the highest-priority candidate before entering Stage 3 (keep n and global_iter_id). Rationale: without this gate both KILOGAS_432 and KILOGAS_353 locked n=4-fixed bests whose release later delivered ΔBIC_eff −1063 / −465.
5. If s\* is a degenerate state (parameters on bounds, bulge/disk fluxes identical), do not force Stage 3; instead inject "repair the degeneration" as a hard constraint into `generate_galfit_beam_actions`'s `local_state_description` and restart one beam search round (reset Q and stagnation; keep n and global_iter_id).
...
**Generation rules for recovery candidates** (the orchestrator executes them in Step d.ii alongside the other d.ii checks; for non-swap FAILs):
...
**Progressive relaxation (level 3; mandatory, not optional)**: if A or B passes but a component still hits its **tightened** cap with no degeneracy signal, the cap is a **self-imposed** bound (current re_max < original input re_max, Tier 1 of the 🔑 Bound-Relaxation Rule) — progressive relaxation follows the standard `tune(component, re_max = hit value × 1.3)` action, and the relaxation candidate is mandatory (the VLM's, or the orchestrator's supplement with g ≥ 0.5).
...
**Relation to the VLM**: the protocol does not replace the VLM — Step d.i still runs normally with visual candidates and FAIL repairs (possibly PA fixes, component changes). Recovery candidates are merely one more d.ii rule, parallel to the other numeric checks. Both classes are scored and enqueued together in Step f; the beam search's parallel exploration is fully preserved.
...
2. **Physical-plausibility prior**: consistency with the "Disk → (F1 if detected) → Bulge → Bar → Other" addition order; conformity of Bar/Bulge/AGN with the recognition conditions (see `<Overall workflow of galaxy component analysis>`; AGN (psf) is the sole legal central point-source slot). **Stage-1 detect_bar_lopsidedness results are only a weak prior here**: a detection may add a little (hint-level positive evidence), but **a non-detection must not deduct** — non-detection is zero evidence, not negative. A Bar/Fourier candidate grounded in residual evidence (central quadrupole, high-ellipticity inner structure, bar-like residuals, etc.) scores on the **strength of the residual evidence** even if Stage 1 detected nothing; the gold standard is residual-driven fitting validation, not Stage-1 detection.
...
6. **BIC threshold**: active only when the action adds/removes an AGN (psf); estimate whether ΔBIC clears the +10 threshold (BIC = `fit_statistics`'s `bic_eff` per the BIC Convention; fall back to `bic1d` if missing).
...
- **Bulge n-release candidate (depth ≥ 3)**: when the parent state contains a bulge whose n is still fixed (the depth ≤ 2 skeleton scaffold) and the beam is generating candidates for depth ≥ 3, the `tune(bulge, n free)` candidate (the VLM's own per its depth-staged Bulge-n rules, or an "[orchestrator n-release supplement]" generated per §Faithful-Execution Principle B-fills when the VLM omits it or files only a waiver) must be enqueued with a floor of **g ≥ 0.5**, initial value = the parent's n (typically 4), `.cons` band written as `2 n 0.5 to 8` in the enforced `to` form. Rationale: releasing n adjudicates **classical bulge (n≈4) vs pseudobulge (n≈1–2)** — two distinct physical hypotheses — and is the single cheapest, highest-information parameter experiment in the space (real incidents: KILOGAS_432 n 4→1.38 at ΔBIC_eff −1063; KILOGAS_353 n 4→0.60 at ΔBIC_eff −465 — both searches had deprioritised or mis-encoded it and locked an n=4-fixed best). The release test must be executed on a valid round (`convergence.flag = ok`, `to`-form bands) before an n=4-fixed state may be locked as final (see Step 3.5).
...
- **Disk-Re bottleneck candidate** (see d.ii): when the bar Re hits the cap with Re degeneracy (≥0.85) or flux-approaching (≥83% disk), the `tune(disk, larger Re)` candidate is floor-enqueued at g ≥ 0.5.
...
- **`global_state_description` (global state)**: a **distillation** of cross-round stable facts (not the full working_note!). Fixed schema, fixed field order, ≤ ~50 lines total:
  ...
  [Meta] Pixel contract: every Re/position in this file is px, in the same reference frame as the VLM's panels and the feedme parameter rows — directly diffable and directly writable (the orchestrator fills values verbatim, no unit conversion); arcsec is forbidden.
        PSF: FWHM=… px, A_psf=… px² (measured once from the PSF image before the first fit; feeds the default Re lower bound)
  ...
  [State ledger] one line per fitted state (the VLM must compare expected_C' landed signatures line by line before generating each candidate):
      ...
      | A.4 | {disk:Rs6.5(Re10.9),M16.2; bulge:n4f,Re1.2px,M18.7; agn:psf,M19.5} | 23499 | PASS | bulge n release pending |
  ...
  [Verified basins] px values + source rounds + evidence tier ([data-verified]/[unverified]).
      e.g. bar PA ≈ 45°, Re ≈ 5.2px; anchored in A.4/A.6/A.8 [data-verified]
  [Refuted hypotheses] all five fields mandatory: direction | context signature | quantitative evidence | failure reason | reopening condition.
      e.g. bar+bulge coexistence | {disk,bulge,bar,agn} | ΔBIC_eff +15.5/+67.5 (A.5/A.11) | the two components degenerate at Re≈1.7px, fighting for flux | the degeneracy disappears
  ...
  - `[Refuted hypotheses]`: new entry when the BIC worsened by ≥ 10 in the same context or the physicality FAILED — **with ΔBIC/bound-hit numbers + failure reason + reopening condition**. **Validity prerequisite (hard)**: entries may be grounded **only on valid rounds** — the round's `fit_statistics.convergence.flag` must be `ok` and its `.cons` bands must have passed `check_feedme_file`'s encoding validation; a [sub-converged] round's outcome is inadmissible evidence (see the Refutation validity rule in the formal definitions).
- **`local_state_description` (current-round supplement)**: the round's objective description, containing:
  ...
  2. **The concrete problems of the current fit** (the most important part; objective and thorough):
     ...
     - **disk-Re bottleneck signal** (when the parent has a bar): bar `Re`/`re_max`/bound-hit status + disk `Re`/`re_max`/bound-hit status; on a hit, mark "disk-Re bottleneck hit" — numbers only;
  ...
  3. The orchestrator's numeric-rule delegations (disk-Re bottleneck numbers / flux-misallocation signals A/B/D / other d.ii quantitative signals).
...
- Component type (the `0)` line: `sersic` / `expdisk` / `psf`)
- The `# STRUCTURE:` semantic name (the target's name; new components are named by physical type disk/bulge/bar/agn)
...
- **Re triplet written directly (galfit has no conversion)**: a Re in a VLM candidate is a **pixel value** (even a stray `"` mark means pixels — the VLM reads a pixel grid); the feedme `4)` row is pixels too — write the triplet's **Re_init verbatim into the `4)` row** and `Re_min`/`Re_max` into the `.cons` `re` line in the **absolute `to` form**: `<number>  re  <Re_min> to <Re_max>` (⚠️ empirical: a bare two-number row is parsed as input-relative offsets and never enforces the absolute band — see CLAUDE.md's `.cons` numeric-band semantics note). **Sole exception**: for an expdisk (Disk) target, the `4)` row takes the scale length `Rs = Re / 1.68` (the triplet refers to the effective radius). **Any arcsec conversion is forbidden; calling unit-conversion tools is forbidden.**
- **SingleSersic→Disk conversion (mandatory when the decomposition begins; B-fill mechanics for `tune(singlesersic→disk: expdisk)`)**: the moment a second central component is added, the starting `singlesersic` (sersic, free n) **must** be converted to the disk slot — type → `expdisk`, `# STRUCTURE:` name → `disk`, the `4)` row takes `Rs = parent fitted Re / 1.68`, the n row is dropped (expdisk has none), and mag/q/PA/centre inherit the parent converged values (warm-start). Without this conversion the child model contains a sersic disk with free n — an **invalid solution-space state** (`check_feedme_file` rejects it: "the disk slot in a multi-component model must be expdisk"). Conversely, when removals leave exactly one central component, reverting `expdisk disk → sersic singlesersic` (n free) is legal (the elliptical terminal state).
- Other component centres (e.g. an added Bulge's x/y): usually the parent fitted value (concentric) or the main-galaxy centre pixel — not the VLM's rough value.
...
**Legitimate adjustment range of Class B**: even for Class-B fills, the orchestrator must not alter the candidate's physical intent. If the VLM proposes `add(Bulge, Re_init=1px)`, the orchestrator may set Re_init to 0.8px or 1.2px (same order) but not 0.3px (three times off — turning an "extended core" into a "compact point source").

**Canonical counter-examples (strictly forbidden)**:
- The VLM proposes `add(Bulge, n=4 fixed, Re_init=1px)`; the orchestrator executes `add(Bulge, n=2 free, Re_init=0.3px)` — violating Class A (n value/state) and Class B's range at once
...
**Correct practice**: if the orchestrator thinks the VLM's n=4 is unreasonable, discard the candidate and log "discarded: Bulge n=4 conflicts with the pseudobulge prior"; to test n=2, submit it as a new orchestrator candidate through the full scoring flow — never piggyback on a VLM candidate.
...
- PSF: FWHM = … px, A_psf = … px² (from check_feedme_file before the first fit; feeds the default Re floor)
...
|---|---|---|---|---|---|
| 1 | A | A.2 | +AGN (compact, psf) | 0.55 | 0.78 |
...
|---|---|---|
| A.4 | {disk:Rs6.5(Re10.9); bulge:n4f,Re~1.2px; agn:psf} | _iter4.feedme |

### Result ledger (fitted-state signatures + outcomes)
| round | state signature (px) | BIC_eff | verdict | zombie/bounds |
|---|---|---|---|---|
| A.4 | {disk:Rs6.5(Re10.9),M16.2; bulge:n4f,Re1.2px,M18.7; agn:psf,M19.5} | 23499.2 | PASS | bulge n release pending |
| A.11 | {disk,bar,agn, bulge:[zombie]} | 23527.2 | FAIL | bulge M24 collapsed |
...
- Occam's razor:
  - **AGN**: if s\* has an AGN (psf) with ΔBIC < 10 (BIC_eff; see the BIC Convention), restart the beam search with `remove(AGN)` as the highest-priority candidate for validation; keep the AGN if the BIC worsens on removal. (The AGN is the only point-source component of the solution space; candidates adding a central point source under any other name are invalid and discarded outright.)
...
* **Report skeleton (normative; hard requirement)**: the report must use the following section headers, in this exact order and wording (free-form content within each; extra subsections may be appended inside a section but the five top-level headers below must appear verbatim — downstream automation locates sections by title):
    1. `## Generation time` — date and time.
    2. `## Preprocessing information` — mask notes and the background-setting rationale.
    3. `## Iteration log` — organised from the multi-branch structure of `working_note.md` — per branch (A/B/C…), the evidence for each round's component additions/removals (multimodal visual judgement records), parameter divergences and rollbacks, the beam-truncated candidate action_ids, cross-branch decisions and semantic-dedup merges.
    4. `## Best-result locking analysis and result` — the locking rationale, the final adopted parameter table, and the physical interpretation.
    5. `## Attachment index` — the best round's directory path, the final `feedme`, and the final comparison image (original/model/residual) path.
    Followed by the JSON output block at the very end of the document.
* **JSON output (normative schema; hard requirement — parsed downstream for rule extraction and automation)**: the document must end with a fenced ```json block whose object contains these three required fields **exactly as named and typed** (renaming, retyping or omitting any of them is a spec violation):
  ```json
  ...
  - `components`: **array of strings**, one per physical component in the best round, from the fixed vocabulary `[Disk,Bulge,Bar,Agn,Fourier,SingleSersic]` (e.g. `["Disk","Bulge","Bar"]`; `Agn` = the AGN point-source component) — objects, nested arrays or ad-hoc names are forbidden. (Branch note: Companion/Lens/OuterDisk/edgedisk are outside this branch's solution space and never occur.)
