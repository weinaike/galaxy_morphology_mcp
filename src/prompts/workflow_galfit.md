...
* 步骤1. 拟合与调参：使用 run_galfit，执行星系拟合（要求输入配置文件绝对路径），
  ...
  - 本阶段要遵循component_analysis的分析结果（即成分修改建议），只能够微调调参数和约束、不能够改变component_analysis要求的成分数量和类型。
...
重复执行上述1-2步骤，直到步骤2的 `component_analysis`工具 给出无需调整5类物理成分(盘、核球、侧视盘、棒、AGN)，且当前参数已经合理收敛后，才能进入步骤3. 
...
* 步骤4. 锁定最佳结果与物理意义解读
    - 锁定最佳结果（选择模型中参数符合物理意义且参数收敛的最优轮次，并说明选择的理由）。并给出其对应的形态学物理意义（如：成分 A 代表经典的盘结构，成分 B 代表致密的核心星团）。
    - 基于最佳结果，调用 fourier_mode_analysis 对残差图进行分析判断是否需要采用 fourier mode 对偏心非对称残差拟合。
        - 只能使用 1 阶 Fourier 模式进行拟合补偿。且只能作用于 Disk 成分
        - 最终是否fourier mode，基于奥卡姆剃刀原则进行判断。
...
* **报告内容包含：**
    ...
    * **最佳结果详情：** 最佳结果的锁定理由、最终采用的参数表、物理意义解读。
