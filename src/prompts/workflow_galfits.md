...
d. **候选生成 + 拟合结果物理性判定（无条件硬约束——见 §候选生成的诊断式原则；两个正交来源合并后统一进入步骤 f 打分入队）**：本步骤只要步骤 b 拟合成功就**必须**执行（失败处置分支 b.5 除外），无论 s' 的物理性判定结果如何、BIC 是否反升、参数是否触界、队列是否仍有未消费候选、拟合预算是否紧张。每轮主循环的候选由两个触发条件正交的来源并行生成——d.i 由 VLM 基于残差图像的视觉分析与**物理性判定**驱动，d.ii 由主模型基于 `.gssummary` 的客观阈值驱动。两类候选合并后走完全相同的去重 / 打分 / 截断规则（步骤 f），彼此平等竞争入队。
    ...
    - **d.ii 主模型数值规则驱动候选**：主模型基于 s' 的 `.gssummary` 客观数值检查，把**现象与数值事实**客观报告给 VLM；**生成什么候选完全由 VLM 依据 `beam_action_generation_prompt.md` 的规则自主决定**（主模型不代生成、不在描述中给方向建议）。这类检查针对"视觉上可见但数值上可疑的成分"——VLM 从图像上看到该成分存在会倾向于保留，但数值上若可疑（如通量极低、或基础成分 Re 偏小被延展成分代偿），主模型把客观观数据交给 VLM，由 VLM 结合原图视觉验证决定是否生成调整候选（移除或参数调整）。当前定义的触发规则：
        - **伴星系必要性检查（数值 + 视觉双轴判据）**：若 s' 含 Companion，读取 `.gssummary` 中 companion 与 disk 的 `logNorm_<component>_<band>` / `Mag_<component>_<band>`，计算通量比 `f_companion/f_disk = 10^(−0.4·ΔMag)`（其中 `ΔMag = Mag_companion − Mag_disk`）。
          ...
          - 若通量比 ≤ 1%（**条件 A 命中**）：主模型**不生成任何候选**，只把三项数值事实（通量比、ΔMag、`ΔlogNorm = logNorm_companion − logNorm_disk`）写入当轮 `generate_beam_actions` 的 `local_state_description`，格式为："伴星系条件 A 命中（客观事实）：companion 通量比 = 0.4%, ΔMag = 5.91, ΔlogNorm = -2.37。"——**不附加任何操作建议**。条件 B 视觉验证与 `remove(Companion)` 候选的生成由 VLM 按 `beam_action_generation_prompt.md` §伴星系移除验证复核 自主决定：仅当 A（数值暗）AND B（原图无可见源）同时成立时才生成 remove 候选；原图有可见亮斑则不生成。
        ...
        - **disk Re 瓶颈检查（延展成分触界 + Re/通量简并判据）**：若 s' 含 lens 或 bar（P 块 label 含 `lens`/`bar`），读取其 `Re` 拟合值与 lyric 中对应的 `re_max`，以及 disk 的 `Re` 与 `Mag`。当 lens/bar 的 `Re` 触上限（拟合值 == re_max，或在 re_max 的 2% 范围内）且满足下列**任一**子条件时，判定为"disk Re 瓶颈命中"——延展成分想更大但被 disk Re 拖住，是 disk Re 偏小的客观信号：
          ...
          - **事实报告**（任一子条件命中后，若 disk Re 也触上限则不触发——disk 已无空间）：主模型把瓶颈**数值事实**写入 `local_state_description`，格式为："disk Re 瓶颈信号（客观事实）：lens(bar)_Re=5.0 触上限（re_max=5.0），Re_lens(bar)/Re_disk=0.93 [≥0.85（Re 简并命中）/ <0.85]，lens(bar)_Mag=16.86 [≤ disk_Mag+0.2=17.28（通量接近命中）/ > 该值]。disk_Re=6.1（re_max=X，未触界）。"——**不附加任何操作建议**；`tune(disk, Re 更大)` 等候选的生成（含 1D 曲线视觉确认）由 VLM 按 `beam_action_generation_prompt.md` §Disk 外围光度不足触发规则 自主决定。VLM 生成的该方向候选以 g ≥ 0.5 保底**入队**（强制保留条款，见 §去重与排序——保底只作用于排队打分，不改变生成权归属）。
        ...
        - **lens Re 膨胀检查（参数状态触发，生成三条竞争式候选）**：若 s' 含 lens，读取 lens 的 `Re` 拟合值、lyric 中 lens 的 `re_max`、以及 disk 的 `Re` 与 `Mag`。
          ...
          - **事实报告**（主模型把信号**数值事实**写入 `local_state_description`，**不生成候选、不附加操作建议**——竞争路径 A/B/C（+条件触发的 D）的生成由 VLM 按 `beam_action_generation_prompt.md` §Lens Re 膨胀触发规则 自主决定，主模型仅打分入队）：格式为："Lens Re 膨胀信号（客观事实）：lens_Re=X 触上限（re_max=Y）[和/或] lens_Re=X ≥ disk_Re=Z（Re 全序反置）。lens_Mag=W，disk_Mag=V。边界来源：当前 re_max=Y [自设（输入原始 re_max=R）/ 原始]。简并数值：Re_lens/Re_disk=比值；lens_Mag−disk_Mag=Δ。"——边界来源与简并数值按 §Lens Re 膨胀触发规则 候选 A/B/C/D 适用条件的口径客观报告，供 VLM 判定各路径是否适用。
          - **缺席处置（客观反馈，不代生成）**：若 VLM 在触发条件成立时漏掉候选 A/B/C 中的某条（或 D 条件成立时漏掉 D）且未在 physical_motivation 中说明放弃理由，主模型**不代为生成**（生成决定权完全在 VLM）——处置为：(1) 在 working_note 决策日志记录"[VLM 候选缺席违规]"（缺席项 + 触发数值）；(2) 在下一轮 `local_state_description` 客观附注该事实（如"上轮 D 适用条件成立但候选缺席"），由 VLM 在新证据下自行补正。候选 A 的 re_max 口径 `0.9 × disk_Re`、候选 B 的 disk re_init 口径 `1.3 × disk_Re`、候选 D 的 `触界值 × 1.3` 仅是主模型转写五元组时的数值口径参考（当 VLM 声明该方向时），不构成主模型生成候选的依据。VLM 生成的 A/B/C/D 候选以 g ≥ 0.5 保底**入队**（强制保留条款，见 §去重与排序——保底只作用于排队打分，不改变生成权归属）。
          ...
          - 若 s' 不含 lens，不触发。
        - **通用触界参数扫描（泛化规则，客观事实报告，不生成候选）**：每轮主模型从 s' 的 `.gssummary` 拟合值与 `_iter{n}.lyric` 五元组边界**逐参数比对**（一切成分、一切参数），列出全部触界参数（|拟合值 − 边界| ≤ 2%×|边界| 或相等），并对每个触界参数**客观报告五项事实**：参数名与拟合值（px，遵像素契约）、被触边界值、边界方向（上/下）、**边界来源**（自设 = 当前边界 ≠ 输入 lyric 原始边界，系前轮收紧/修复动作产生；原始 = 与输入一致）、连续触界轮数（从 working_note 状态账本统计）。该清单原文写入 `local_state_description`（格式如："触界参数清单（客观事实）：lens_Re=9.0px 触上限 re_max=9.0px [自设（原始 12.6px）] 连续 2 轮； companion_Re=1.0px 触下界 re_min=1.0px [原始] 连续 4 轮； …"），**禁止附加任何候选方向建议或祈使句**（"请生成/建议生成/确认生成/应放宽"类措辞均违规）——生成义务由 VLM 按 `beam_action_generation_prompt.md` 🔑 触界参数边界放宽规则 的分级（Tier 1 自设必出 / Tier 2 原始竞争 / 例外豁免）自主决定。本扫描取代逐参数的专项保底：lens Re 膨胀检查中的 D 适用条件即本扫描"自设边界"事实在 lens 上的特例。
        - 未来若需扩展其他客观数值触发（如 sky 背景异常、某成分 Mag 异常暗等），按同样模式在此子项追加规则——**只描述现象与数值事实，生成决策一律交 VLM**。
    - **追溯标记**：d.ii 触发的数值检查（无论 VLM 最终是否生成对应候选）在 `working_note.md` 的相应分支小节标注"[主模型数值规则委托]"，记录实测数值（伴星系：通量比/ΔMag/ΔlogNorm；disk Re 瓶颈：lens/bar Re 与 re_max、disk Re 与 re_max、Re 比值、通量比、命中子条件 A/B；lens Re 膨胀：lens Re 与 re_max、disk Re、是否触上限/反置、lens_Mag、disk_Mag、命中的竞争路径 A/B/C/D，其中 D 需记录**当前 re_max 与输入原始 re_max 的来源判定**（自设边界 or 原始边界）与两简并子条件的实测结果）与 VLM 的视觉验证结论。便于后续审计成分保留/移除/参数调整决策的依据。
...
**渐进放宽（等级 3，已并入 §Lens Re 膨胀触发规则的候选 D，必出而非可选）**：若候选 A 或 B 拟合后 PASS 但 lens 仍触**收紧后的上限**，且无简并信号（Re_lens/Re_disk < 0.85 且 Mag_lens > Mag_disk + 0.2），则该情形**就是** d.ii lens Re 膨胀检查中候选 D 的触发场景（自设边界判定：当前 re_max < 输入原始 re_max）——VLM 必产出 D（`tune(lens, re_max = 触界值 × 1.3)`），VLM 漏给时主模型按"[主模型 lens 膨胀补充-D]"保底生成（g ≥ 0.5）。**不再依赖"后续轮次可提议"的弱措辞**——旧措辞在实际运行中被 A/B/C 委托模板的封闭枚举压制，放宽候选从未产出（真实事故）。非 lens 成分的渐进放宽仍按标准 `tune(component, re_max += 2")` 动作处理，适用 🔑 触界参数边界放宽规则。
...
- **lens 自设上限放宽候选 D**（见 d.ii lens Re 膨胀检查）：当 lens Re 触上限、该上限系前轮收紧产生（当前 re_max < 输入原始 re_max）、且两简并子条件均未命中时，候选 D（`tune(lens, re_max = 触界值 × 1.3)`）以 g ≥ 0.5 保底入队（VLM 给出或主模型按"[主模型 lens 膨胀补充-D]"保底）。这类候选针对"触界源于上一轮修复动作的自设边界而非物理边界"的情形——VLM 的诊断被 A/B/C 竞争路径的封闭枚举占据注意力，对"放宽自设边界"方向存在系统性盲区（真实事故：连续两轮触收紧后上限、无简并，放宽候选从未产出），故需数值触发保底保护。
