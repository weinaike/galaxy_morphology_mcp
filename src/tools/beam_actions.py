...
def generate_beam_actions(
    ...
    handoff = (
        ...
        " **执行历史**比对——(R1) 候选转写为假想 lyric 的规范形式（结构 × vary 配置 × 边界带 × 初始值带）"
        "与**输入账本**比对，带内等价 → 丢弃（同一输入重跑无新信息）；(R2) remove-only / 参数 revert /"
        " 边界还原类闭式转移候选的产出状态可精确投影，投影签名与**结果账本**比对（僵尸感知：仅相差"
        " [zombie] 成分的状态等价），严格命中 → 不入队（零成本回滚），仅结构一致 → 标'[疑似近重复]'"
        "（退化惩罚维度记 0）。候选的 novelty_claim 须与账本核对：结构等价且无新参数轴的候选整条丢弃。\n"
