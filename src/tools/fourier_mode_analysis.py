...
- 只考虑 1 阶傅里叶模式（m=1），且只能作用于 Disk 成分
...
def fourier_mode_analysis(
    ...
    prompt_text = (
        ...
        "- 特别关注 Disk 区域：是否存在一侧系统性偏亮、对侧偏暗的偶极模式\n"
        "- 描述边缘区域是否存在壳层（同心弧）或潮汐尾（细长条带）\n\n"
        "步骤2：排除假阳性\n"
        "- 该不对称是否可由旋臂（正负相间螺旋图案）、尘埃带、前景源或 Mask 边缘解释\n"
        "- 该不对称是否可通过调整已有成分参数（PA、q、中心位置）解决\n\n"
        "步骤3：傅里叶模式必要性判断\n"
