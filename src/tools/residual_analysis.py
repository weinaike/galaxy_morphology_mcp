...
1. 高/低动态范围的原图的特征描述
    ...
    - 描述不同动态范围原图的中心星系的的特征， 并推测高概率的存在的星系成分（要提供强特征证据支持）
...
- 基于阶段三的分析，推理下一步动作（最紧迫）：
    ...
    - 必须确认以下内容：
        ...
        - 2. 伴星系是否均已添加到模型中。如果没有，需要给出具体的添加参数，这个不受中心星系增减成分（小于或等于 1 个成分）的约束。【sersic与PSF可选，根据实际情况选择调整】
        - 3. 同心成分之间的<x><y>应该绑定在一起，x与y要同时绑定，不能只绑定单坐标。
        - 4. Disk 应该是用expdisk模型，不应该使用sersic
