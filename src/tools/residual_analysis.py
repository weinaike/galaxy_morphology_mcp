...
# ── Decision-implementation footer (require) ─────────────────────────────
# Kept as two independent complete strings (not split into shared-rule + example):
# the example vocabulary differs by tool — single-band GALFIT uses expdisk,
# multi-band GALFITS has no expdisk (disk is sersic n=1). Both stay in this file
# because they are short and tool-specific.
...
# 决策落实守则
- 必须严格落实【调整决策】中的要求，基于上一轮的拟合结果的基础上调整初始参数。
    - 落实过程中，不可以私自增减（即使因为拟合出现异常），增减成分的决策权归属 analyze_multiband_components 所有。
    - 若【调整决策】涉及到多个成分增减，需要遵循一次只增减一个成分的原则，避免陷入局部最优或者拟合崩溃。
        - example1: 目标：sersic->(sersic n=1 disk + F1 + Bulge);实际要分三步：sersic->sersic n=1 disk->(sersic n=1 disk + F1)->(sersic n=1 disk + F1 + Bulge)
        - example2: 目标：sersic->(sersic n=1 disk + companion);实际要分两步：sersic->sersic n=1 disk->(sersic n=1 disk + companion)
        - 这里的一次是执行一次 run_galfits，一个调整策略可以执行多次run_galfits，调整与执行的优先级持续参考 example1、example2 的顺序。(后步骤要基于前步骤的输出结果参数基础上继续调整)
    - 落实过程中，可以对参数细节做调整，比如参数初值、fix/free等，但必须保证调整的方向和目标与 analyze_multiband_components 输出的决策完全一致。
- 决策落实（多次调参拟合）完成后，无论结果如何，都必须必须向 analyze_multiband_components 汇报进展并获取下一轮的分析并获取调整意见，严禁私自决定。
- 你可以怀疑 analyze_multiband_components 的判断，但只能保留意见，必须严格执行 analyze_multiband_components 的决策，直到所有决策落地执行（可以反馈意见，严禁私自篡改决策）。
'''


def _best_round_verbose(best_info: dict) -> dict:
    """BEST_ROUND_VERBOSE=1 时附加的 verbose 字段，两种模式共用。"""
    out: dict = {}
    if os.environ.get("BEST_ROUND_VERBOSE") == "1":
        if best_info.get("verdict") is not None:
            out["best_round_verdict"] = best_info["verdict"]
        if best_info.get("comparison_text") is not None:
            out["best_round_comparison"] = best_info["comparison_text"]
    return out


def _run_component_analysis_pipeline(
    *,
    image_file: str,
    summary_file: str,
    summary_content: str,
    custom_instructions: str,
    working_note_file: str,
    component_spec: str,
    require: str,
    lyric_file: str | None = None,
    fit_tool_label: str = "GALFIT",
) -> dict[str, Any]:
    """Shared backend for component_analysis (single-band) and
    analyze_multiband_components (multi-band).

    Tool/mode-specific inputs — component spec, summary text, require footer,
    fit-tool label — are resolved by the caller; everything else (system message,
    best-round memory, cc/acp/vlm dispatch, persistence, timing) is identical.
    On success the raw ``_best_info`` is stashed under key ``"_best_info"`` for
    the caller to format into its own return shape (single vs multi band differ).
    """
    # Maintain best-round memory for this galaxy (visual-primary VLM comparison).
    _best_info = _brr.update_best_round_for_call(
        image_path=image_file, summary_path=summary_file, lyric_file=lyric_file)
    ...
    if analysis_mode == "cc":
        ...
        step1 = f'''
你是一个集成了"计算机视觉特征提取"与"天体物理形态学专家推理"的自动化诊断 Agent。你的任务是基于 {fit_tool_label} 的拟合结果，通过严密的四步思维链（Chain-of-Thought），诊断当前模型的缺陷，并输出下一步的调整决策。
...
def analyze_multiband_components(
    ...
    result = _run_component_analysis_pipeline(
        image_file=comparison_file,
        summary_file=summary_file,
        summary_content=summary_content,
        custom_instructions=custom_instructions,
        working_note_file=working_note_file,
        component_spec=component_spec,
        require=_REQUIRE_GALFITS,
        lyric_file=lyric_file,
        fit_tool_label="GALFITS",
    )
    if result.get("status") != "success":
        return result

    _best_info = result.pop("_best_info", None)
    if _best_info is not None:
        ...
        result.update(_best_round_verbose(_best_info))
    return result
...
def component_analysis(
    ...
    result = _run_component_analysis_pipeline(
        ...
        working_note_file=working_note_file,
        component_spec=component_spec,
        require=_REQUIRE_GALFIT,
        lyric_file=None,
        fit_tool_label="GALFIT",
    )
    if result.get("status") != "success":
        return result
